/*
  $Id: malloc.c,v 1.6 2007/03/26 20:47:03 mark Exp $
...
/* This define wraps the changes done by TransGaming to result in somewhat
   of a frankenstein (half Unix, half Windows) allocator. The primary change
   being done is to use VirtualAlloc() and friends instead of mmap(). In
   addition, some APIs have been expanded */
#define TGCHANGES
...
/*
  ...
  spaces.
*/
void* mspace_realloc(mspace msp, void* mem, size_t newsize, int inplaceonly);
...
#if !defined(WIN32) && !defined(TGCHANGES)
...
/* ... and additional padding for fake next-chunk at foot, plus
   a malloc_segment structure */
#define MMAP_FOOT_PAD       (FOUR_SIZE_T_SIZES + FOUR_SIZE_T_SIZES)
...
/* this is a variation on malloc_segment which is used for allocations which
   are > mmap_threshhold and are thus directly mmap'd. As a consequence, they
   don't appear in the mstate->seg list */

struct large_malloc_segment {
  char*        base;                   /* base address */
  size_t       size;                   /* allocated size */
  struct large_malloc_segment* next;   /* ptr to next segment */
  struct large_malloc_segment* prev;   /* ptr to previous segment */
};

typedef struct large_malloc_segment* lmsegmentptr;
...
#endif /* USE_LOCKS */
  msegment   seg;
  lmsegmentptr large_seg; /* list of directly mmap'd large allocations */
  size_t     exts;
};
...
#define HEAP_GROWABLE_BIT (8U)
#define heap_growable(M)      ((M)->mflags &   HEAP_GROWABLE_BIT)
#define enable_growable(M)    ((M)->mflags |=  HEAP_GROWABLE_BIT)
#define disable_growable(M)   ((M)->mflags &= ~HEAP_GROWABLE_BIT)
...
#if MORECORE_CONTIGUOUS
    mparams.default_mflags = USE_LOCK_BIT|USE_MMAP_BIT|HEAP_GROWABLE_BIT;
#else  /* MORECORE_CONTIGUOUS */
    mparams.default_mflags = USE_LOCK_BIT|USE_MMAP_BIT|USE_NONCONTIGUOUS_BIT|HEAP_GROWABLE_BIT;
#endif /* MORECORE_CONTIGUOUS */
...
static void* mmap_alloc(mstate m, size_t nb) {
  size_t mmsize = mmap_align(nb + MMAP_CHUNK_OVERHEAD + MMAP_FOOT_PAD +
                             CHUNK_ALIGN_MASK);
  if (mmsize > nb) {     /* Check for wrap around 0 */
    ...
    if (mm != CMFAIL) {
      ...
      lmsegmentptr lseg =
          (lmsegmentptr)((char *)p + psize + FOUR_SIZE_T_SIZES);
      ...
      lseg->base = chunk2mem (p);
      lseg->size = psize;
      lseg->prev = 0;
      lseg->next = m->large_seg;
      if (m->large_seg)
          m->large_seg->prev = lseg;
      m->large_seg = lseg;
...
static mchunkptr mmap_resize(mstate m, mchunkptr oldp, size_t nb) {
  ...
  if (oldsize >= nb + SIZE_T_SIZE)
    ...
    if (cp != CMFAIL) {
      ...
      lmsegmentptr lseg =
          (lmsegmentptr)((char *)newp + psize + FOUR_SIZE_T_SIZES);
      lseg->base = chunk2mem (newp);
      lseg->size = psize;
      lseg->prev = 0;
      lseg->next = m->large_seg;
      if (m->large_seg)
...
size_t destroy_mspace(mspace msp) {
  ...
  if (ok_magic(ms)) {
    ...
    lmsegmentptr lseg = ms->large_seg;

    while (lseg)
    {
        char *base = lseg->base;
        lseg = lseg->next;
        mspace_free (msp, base);
    }
...
void* mspace_realloc(mspace msp, void* oldmem, size_t bytes, int inplaceonly) {
  if (oldmem == 0)
  ...
  if (bytes == 0) {
    ...
    return internal_realloc(ms, oldmem, bytes, inplaceonly);
  }
}
...
int mspace_validate (mspace msp, const void *mem)
{
   mchunkptr p  = mem2chunk(mem);
   mstate ms = (mstate)msp;
   msegmentptr pSeg;
   ...
   /* Walk segments and see if the pointer is in one of them */
   pSeg = segment_holding (ms, (char *)p);
   ...
   if (!pSeg)
   {
       ...
       for (lseg = ms->large_seg; lseg; lseg = lseg->next)
       {
           if (((char *)mem >= lseg->base) &&
               ((char *)mem < (lseg->base + lseg->size)))
           {
               found = 1;
               break;
           }
       ...
       /* Check at least the data structure fits, so we can try to
          dereference its members */
       if (((char *)p + sizeof (mchunk)) >= (lseg->base + lseg->size))
          return 0;

       /* Check the whole chunk fits and we don't wrap around */
       ChunkSize = chunksize (p);
       if ((ChunkSize > HALF_MAX_SIZE_T) ||
           (((char *)p + ChunkSize) > (lseg->base + lseg->size)) ||
           (((char *)p + ChunkSize) < (char *)p))
          return 0;
   }
   else
   {
      ...
      /* Check at least the data structure fits, so we can try to
         dereference its members */
      if (((char *)p + sizeof (mchunk)) >= (pSeg->base + pSeg->size))
         return 0;

      /* Check the whole chunk fits and we don't wrap around */
      ChunkSize = chunksize (p);
      if ((ChunkSize > HALF_MAX_SIZE_T) ||
          (((char *)p + ChunkSize) > (pSeg->base + pSeg->size)) ||
          (((char *)p + ChunkSize) < (char *)p))
         return 0;
   }

   /* Check that mspace associated with mem == msp; we must have already
      determined that it is safe to dereference p and the chunksize is
      at least semi-valid */
#if FOOTERS
   stored_ms = get_mstate_for (p);
   if (stored_ms != ms)
      return 0;
#endif
