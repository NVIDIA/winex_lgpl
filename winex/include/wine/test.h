...
/* Macro to skip and mark a set of tests as TODO */
#define todo_skip_nonwin_START todo("cider") {   \
    if (strcmp(winetest_platform,"windows")!=0) { \
       skip("Skipping todo tests\n");           \
       continue;                                \
    }

#define todo_skip_win_START todo("windows") {     \
    if (strcmp(winetest_platform,"windows")==0) { \
       skip("Skipping todo tests\n");             \
       continue;                                  \
    }

#define only_cider_START todo("windows") {      \
    if (strcmp(winetest_platform,"cider")!=0) { \
       skip("Skipping tests\n");                \
       continue;                                \
    }

#define only_gttv_START todo("windows") {      \
    if (strcmp(winetest_platform,"gametreetv")!=0) { \
       skip("Skipping tests\n");                \
       continue;                                \
    }

#define todo_skip_win_END }
#define todo_skip_nonwin_END }
#define only_cider_END }
#define only_gttv_END }
...
{
    for (; str1[0] != 0 && str1[0] == str2[0]; str1++, str2++);
...
static int run_test( const char *name )
{
    ...
    successes = failures = todo_successes = todo_failures = skipped = 0;
    tls_index=TlsAlloc();
    current_test = test;
    test->func();
...
int main( int argc, char **argv )
{
    ...
    if ((p = getenv( "WINETEST_PLATFORM" )))
    { 
        winetest_platform = strdup(p);
    }
    else
    {
        typedef LPCSTR (WINAPI* LPTGGETOSFN)(void);
        LPTGGETOSFN pTGGetOS = NULL;
        HMODULE ntdll = GetModuleHandle("ntdll.dll");
        
        if (ntdll) pTGGetOS = (LPTGGETOSFN)GetProcAddress(ntdll, "TGGetOS");
        
        if (pTGGetOS)
        {
            p = pTGGetOS();
            
            /* TODO add granularity to the platform variable */
            if (!strcmp(p, "MacOSX") ||
                !strcmp(p, "GTTV") ||
                !strcmp(p, "Linux")) 
                winetest_platform = "cider";
            else 
