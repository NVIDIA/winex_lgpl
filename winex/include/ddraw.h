...
typedef struct _DDPIXELFORMAT {
    ...
    DWORD	dwFourCC;               /* 8: FOURCC code (could be a D3DFMT) */
    union {
	...
	DWORD   dwPrivateFormatBitCount;/* how many bits for D3DFMT (when D3DFORMAT is set) */
    } DUMMYUNIONNAME1;
    union {
	...
	DWORD   dwOperations;           /* used when D3DFORMAT is set */
    } DUMMYUNIONNAME2;
    union {
...
typedef struct _DDSURFACEDESC2
{
	...
	union {
		DWORD	dwBackBufferCount;/* 14: number of back buffers requested*/
		DWORD	dwDepth;	  /* 14: used for volume textures */
	} DUMMYUNIONNAME5;
	union {
		DWORD	dwMipMapCount;/* 18:number of mip-map levels requested*/
		DWORD	dwRefreshRate;/* 18:refresh rate (used when display mode is described)*/
