...
static BOOL gdb_startup(struct gdb_context* gdbctx, DEBUG_EVENT* de, unsigned flags)
{
    ...
    if (flags & 1)
		...
		/* Always at least do a bt and info threads */
                fprintf(f, "info threads\n");
                fprintf(f, "bt\n");
                ...
                if (flags & 2)
                    /* Should probably use TERM env variable rather than xterm, but oh well */
                    execlp("xterm", "xterm", "-e", gdb_path, "-x", buf, NULL);
                else
                    execlp(gdb_path, gdb_path, "-x", buf, NULL);

		/* Reached only if exec fails */
                fprintf( stderr, "Unable to start gdb! Please make sure you have gdb present on your system:"
				 "flags=0x%x gdb_path=%s\n", flags, gdb_path );
                assert(0);
                break;
            }
            break;
