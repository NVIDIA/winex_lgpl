...
char newline[] = "\n";
char version_string[] = "WCMD Version 0.18\n\n";
...
DWORD count;
HANDLE h;
BOOL has_options = FALSE; /* param should only contain strings after /options */
 ...
 */
  ...
  if ((strstr(args, "/q") != NULL) || (strstr(args, "/Q") != NULL)) {
    WCMD_echo ("OFF");
  }

  if ((strstr(args, "/c") != NULL) || (strstr(args, "/C") != NULL)) {
    WCMD_process_command (param);
    return 0;
  }

  if ((strstr(args, "/k") != NULL) || (strstr(args, "/K") != NULL)) {
    WCMD_process_command (param);
  }
