...
static void write_type_v(FILE *h, type_t *t, int is_field, int declonly, const char *name, int use_comma)
{
  type_t *pt;
  int ptr_level = 0;
  ...
  if (use_comma) fprintf(h, ",%s", name);
  else if (name) fprintf(h, "%s%s", needs_space_after(t) ? " " : "", name );
...
{
  write_type_v(f, t, field, FALSE, name, FALSE);
}

void write_type_decl(FILE *f, type_t *t, const char *name)
{
  write_type_v(f, t, FALSE, TRUE, name, FALSE);
}
...
static void write_name(FILE *h, const var_t *v)
{
  if (is_attr( v->attrs, ATTR_PROPGET ))
    fprintf(h, "get_" );
  else if (is_attr( v->attrs, ATTR_PROPPUT ))
...
static void write_icom_method_def(FILE *header, const type_t *iface)
{
  const statement_t *stmt;

  if (type_iface_get_inherit(iface))
    fprintf(header, " \\\n    %s_IMETHODS", type_iface_get_inherit(iface)->name);
  ...
  STATEMENTS_FOR_EACH_FUNC(stmt, type_iface_get_stmts(iface))
  {
    ...
    if (!is_callas(func->attrs)) 
    {
      ...
      write_name(header, func);
      if (type_get_function_args(func->type))
      {
        LIST_FOR_EACH_ENTRY( arg, type_get_function_args(func->type), const var_t, entry )
        {
          fprintf(header, ",");
          write_type_v(header, arg->type, FALSE, TRUE, arg->name, TRUE);
        }
      }
      fprintf(header, ")");
    }
...
static void write_com_interface_end(FILE *header, type_t *iface)
{
  ...
  write_icom_method_def(header, iface);
  fprintf(header, "\n\n");

  fprintf(header, "#if !defined(__cplusplus) || defined(COBJMACROS) || defined(CINTERFACE)\n");
