...
static void ParseDebug(void)
{
    ...
    for (;;)
    {
        token = GetToken(0);
        if (*token == ')') break;
        if ( find_debugchannel(token) > -1 )
        {
            warning( "Duplicate debug channel '%s' defined\n", token );
            continue;
        }
...
static void ParseExportFunction( ORDDEF *odp )
{
    ...
    /* The last element of the line should be the link name. If it is not, then we have an error. */
...
static int name_compare( const void *name1, const void *name2 )
{
    ...
    return strcmp( odp1->export_name, odp2->export_name );
}
...
static void sort_names(void)
{
    ...
    {
        /* technically the same function name may be exported twice with a different calling
           convention each time (ie: cdecl vs stdcall) as long as the decorated names are
           used.  To prevent false positives from being hit for this case we'll use the
           export name of each function when comparing instead of the internal name. */
        if (!strcmp( Names[i]->export_name, Names[i+1]->export_name ))
        {
            current_line = max( Names[i]->lineno, Names[i+1]->lineno );
            fatal_error( "'%s' redefined (previous definition at line %d)\n",
                         Names[i]->export_name, min( Names[i]->lineno, Names[i+1]->lineno ) );
        }
    }
}
