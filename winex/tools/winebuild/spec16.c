                          ...
                          int max_data_offset )
{
    ...
    pFileInfo->cBytes = sizeof(*pFileInfo) - sizeof(pFileInfo->szPathName)
                        + strlen(DLLFileName);
    strcpy( (char *)pFileInfo->szPathName, DLLFileName );
    pstr = (BYTE *)pFileInfo + pFileInfo->cBytes + 1;

      /* Segment table */

    pstr = (BYTE *)(((long)pstr + 3) & ~3);
    ...
    *pstr = strlen( DLLName );
    ...
    for (i = 1; i <= Limit; i++)
    {
        ...
        *pstr = strlen( odp->name );
        strcpy( (char *)(pstr + 1), odp->name );
        strupper( (char *)(pstr + 1) );
        pstr += *pstr + 1;
        PUT_UA_WORD( pstr, i );
        pstr += sizeof(WORD);
    }
    ...
    *pstr++ = 0;
    ...
    pstr = (BYTE *)(((long)pstr + 3) & ~3);
    dump_bytes( outfile, (unsigned char *)pModule, (int)pstr - (int)pModule,
                "Module", 0 );
...
#endif
                     odp->link_name,
                     (int)((type-typelist)*sizeof(CALLFROM16) -
