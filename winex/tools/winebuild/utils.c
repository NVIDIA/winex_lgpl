...
int get_alignment(int alignBoundary)
{
    ...
    if (target_compiler == PLATFORM_WINDOWS)
...
void output_function_name (FILE *outfile, const char *pName)
{
#ifdef TYPE_ASM_SUPPORTED
   fprintf (outfile, "    \"\\t.type " __ASM_NAME("%s") ",@function\\n\"\n",
            pName);
#elif defined(NEED_TYPE_IN_DEF)
   fprintf (outfile, "    \"\\t.def " __ASM_NAME("%s") "; .scl 2; .type 32; .endef\\n\"\n",
            pName);
#endif
}
