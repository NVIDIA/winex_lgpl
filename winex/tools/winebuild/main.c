...
static void do_pic(void)
{
    UsePIC = 1;
#ifdef __APPLE__
   /* FIXME - hack until we properly generate PIC code on the Mac,
      particularly for the 16-bit thunks */
    if (exec_mode == MODE_RELAY16)
       UsePIC = 0;
#endif
}
...
#ifdef __APPLE__
    /* Hack until we fix up code gen on the Mac */
    UsePIC = 0;
#endif
}
