...
int nb_lib_paths = 0;
int delay_initialization = 0;
int size_of_image = 0;
int need_force_init = 0;
...
char **lib_path = NULL;
enum target_type target_compiler = PLATFORM_UNSPECIFIED;
...
static void do_force_init(void);
static void do_target( const char *arg );
...
static const struct option_descr option_table[] =
{
    ...
    { "-target",  1, do_target  ,"-target          Target compiler" },
    { "-need_force_init", 0, do_force_init, 
      "-need_force_init (MACOSX only) - Specifies the spec file needs a forced init function (for DLLs built as a dylib)" },
    { NULL,       0, NULL,      NULL }
};
...
static void do_target( const char *arg )
{
    if (strcmp(arg, "mingw32")==0)
        target_compiler = PLATFORM_WINDOWS;
}
