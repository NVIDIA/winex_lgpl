...
int main(int argc,char *argv[])
{
...
#ifdef __APPLE__
        /* Pass along apple platform define to allow for platform specific resources */
	add_cmdline_define("__APPLE__=1");
#endif
	...
	if(!no_preprocess && !binary)
	{
		...
		if (fseek (inputFILE, 0L, SEEK_SET))
			error("Couldn't move to beginning of temporary file\n");
	}
	else
	{
		inputFILE = fopen (input_name, "rb");
		if (!inputFILE)
	...
	else
	{
		...
		resource_top = read_resfile(inputFILE);
		if(create_s)
		{
			chat("Writing .s-file");
			write_s_file(output_name, resource_top);
...
static void segvhandler(int sig)
{
	fprintf(stderr, "\n%s:%d: Oops, segment violation\n", input_name ? input_name : "", line_number);
	fflush(stdout);
	fflush(stderr);
	abort();
}
