...
{
    IMAGE_EXPORT_DIRECTORY	*exportDir = get_dir(IMAGE_DIRECTORY_ENTRY_EXPORT);
    unsigned int		i;
    DWORD*			pFunc;
    DWORD*			pName;
    WORD*			pOrdl;
...
{
    IMAGE_IMPORT_DESCRIPTOR	*importDesc = get_dir(IMAGE_DIRECTORY_ENTRY_IMPORT);
    unsigned			nb_imp, i;

    if (!importDesc)	return;
    nb_imp = nt_headers->OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_IMPORT].Size /
	sizeof(*importDesc);
    if (!nb_imp) return;

    printf("Import Table size: %lu\n",
	   nt_headers->OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_IMPORT].Size);/* FIXME */
...
{
    IMAGE_DEBUG_DIRECTORY*	debugDir = get_dir(IMAGE_DIRECTORY_ENTRY_DEBUG);
    unsigned			nb_dbg, i;

    if (!debugDir) return;
    nb_dbg = nt_headers->OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_DEBUG].Size /
	sizeof(*debugDir);
    if (!nb_dbg) return;
...
{
    const IMAGE_RESOURCE_DIRECTORY *root = get_dir(IMAGE_DIRECTORY_ENTRY_RESOURCE);
