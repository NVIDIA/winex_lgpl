...
/* Note that this may be problematic with some compilers; however, we're very unlikely
 to encounter this function in anything modern */
typedef INT     (CALLBACK *ThunkProc_t)();

DWORD WINAPIV SSCall(
	DWORD nr,	/* [in] number of argument bytes */
	DWORD flags,	/* [in] FIXME: flags ? */
	ThunkProc_t fun,	/* [in] function to call */
	...		/* [in/out] arguments */
) {
    DWORD i,ret;
 ...
 */
typedef INT (CALLBACK *BackProc_t)(DWORD, DWORD, DWORD, DWORD, DWORD, DWORD, DWORD, DWORD, DWORD, DWORD);
void WINAPI W32S_BackTo32( CONTEXT86 *context )
{
    LPDWORD stack = (LPDWORD)context->Esp;
    BackProc_t proc = (FARPROC)context->Eip;
