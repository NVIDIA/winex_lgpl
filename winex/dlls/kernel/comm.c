...
static void COMM_WaitCommEventService(async_private *ovp)
{
    ...
    LPOVERLAPPED lpOverlapped = commio->async.lpOverlapped;
    ...
    LPOVERLAPPED lpOverlapped) /* [in/out] for Asynchronous waiting */
{
    ...
    ovp->async.hStartupEvent = CreateEventA (NULL, TRUE, FALSE, NULL);
    if (ovp->async.hStartupEvent == INVALID_HANDLE_VALUE)
    {
...
{
    typedef BOOL (WINAPI *SERIALUI_CommConfigDialogProc)(LPCSTR, HWND, LPCOMMCONFIG);

    SERIALUI_CommConfigDialogProc lpfnCommDialog;
...
{
    typedef BOOL (WINAPI *SERIALUI_SetDefaultCommConfigProc)(LPCSTR, LPCOMMCONFIG, DWORD);

    SERIALUI_SetDefaultCommConfigProc lpfnSetDefaultCommConfig;
    HMODULE hConfigModule;
    BOOL r;
