...
WINE_DEFAULT_DEBUG_CHANNEL(cert);
...
#ifdef WRITE_CERTS_TO_PDRIVE
void WINAPI Cert_SaveCert(PCCERT_CONTEXT cert, const char *dir)
{
    static const int s_certNameSize = 256;
    char certName[s_certNameSize] = "unknown";
    char certSHA1[256] = "failed";
    BYTE sha1Value[20];
    ...
    if (CertGetCertificateContextProperty(cert, CERT_SHA1_HASH_PROP_ID, &sha1Value, &sha1Size))
    {
    ...
    /* get name */
    if (CertGetNameStringA(cert, CERT_NAME_FRIENDLY_DISPLAY_TYPE, 0, NULL, certName, s_certNameSize))
    {
        certName[s_certNameSize - 1] = 0;
        for (i=0; i<strlen(certName); i++)
        {
            if (certName[i]!=' ' && certName[i]!='-' && certName[i]!='.' && !isalpha(certName[i]))
                if (certName[i]<'0' || certName[i]>'9')
                    certName[i]='_';
        }
        while(certName[strlen(certName)-1] == '_' )
            certName[strlen(certName)-1]=0;
    }

    /* create directories in p_drive */
    CreateDirectoryA("p:\\crypt32-certificates\\", NULL );
    snprintf(temp, sizeof(temp), "p:\\crypt32-certificates\\%s", dir);
    CreateDirectoryA(temp, NULL );
    snprintf(temp, sizeof(temp), "p:\\crypt32-certificates\\%s\\%s", dir, certName);
    CreateDirectoryA(temp, NULL );

    /* write to p_drive */
    snprintf(temp, sizeof(temp), "p:\\crypt32-certificates\\%s\\%s\\%s.cer", dir, certName, certSHA1);
    hFile = CreateFileA(temp, GENERIC_WRITE, 0, NULL, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
    if (hFile == INVALID_HANDLE_VALUE)
    {
        ERR("Faled to write certificate \"p:\\crypt32-certificates\\%s\\%s\\%s.cer\"\n", dir, certName, certSHA1);
    }
    else
    {
        /* write file */
        WriteFile(hFile, cert->pbCertEncoded, cert->cbCertEncoded, NULL, NULL);
        CloseHandle(hFile);
    }
}
#endif
 ...
 DWORD dwAddDisposition, PCCERT_CONTEXT *ppCertContext)
{
    ...
    if (cert)
    {
#ifdef WRITE_CERTS_TO_PDRIVE
        char temp[256];
        char storename[256]="unknown";
        void *pvData = storename;
        DWORD cbData = sizeof(storename);
        ...
        {
            PCCERT_CONTEXT cert = CertCreateCertificateContext(X509_ASN_ENCODING, pbCertEncoded, cbCertEncoded);
            Cert_SaveCert(cert, "CertAddEncodedCertificateToStore");
 ...
 const BYTE *pbCertEncoded, DWORD cbCertEncoded)
{
    ...
    {
        PCCERT_CONTEXT cert = CertCreateCertificateContext(X509_ASN_ENCODING, pbCertEncoded, cbCertEncoded);
        Cert_SaveCert(cert, "CertAddEncodedCertificateToSystemStore");
        CertFreeCertificateContext(cert);
    }
#endif
 ...
 const BYTE *pbCertEncoded, DWORD cbCertEncoded)
{
    ...
    {
        PCCERT_CONTEXT cert = CertCreateCertificateContext(X509_ASN_ENCODING, pbCertEncoded, cbCertEncoded);
        Cert_SaveCert(cert, "CertAddEncodedCertificateToSystemStoreW");
        CertFreeCertificateContext(cert);
    }
#endif
...
#ifdef WRITE_CERTS_TO_PDRIVE
    Cert_SaveCert(cert, "CertCreateCertificateContext");
#endif
...
BOOL WINAPI CertFreeCertificateContext(PCCERT_CONTEXT pCertContext)
{
    ...
    if (pCertContext && pCertContext->hCertStore)
    {
        LONG refcount;
        WINECRYPT_CERTSTORE *hcs = pCertContext->hCertStore;

        refcount = Context_GetRef((void *)pCertContext, sizeof(CERT_CONTEXT));

        if ((hcs->dwFlags & CERTSTORE_FLAGS_DEFERING_CLOSE) && refcount == 1)
            CertCloseStore(pCertContext->hCertStore, 0);
    }
 ...
 DWORD dwFlags, const void *pvPara)
{
    ...
    TRACE_(cert)("(%p) (%d) (%d) (%p = %d)\n", pCertContext, dwType, dwFlags, id, id->dwIdChoice);
    ...
    case CERT_ID_KEY_IDENTIFIER:
    {
        ...
        TRACE_(cert)("CERT_ID_KEY_IDENTIFIER: %d -> %d\n", size, id->u.KeyId.cbData);
 ...
 const CERT_SIGNED_CONTENT_INFO *signedCert)
{
    ...
    {
        if (info)
            ERR("CryptFindOIDInfo failed %d\n", info->dwGroupId);
        else
            ERR("CryptFindOIDInfo failed\n");
        SetLastError(NTE_BAD_ALGID);
        return FALSE;
    }
    hashID = info->u.Algid;
    ...
    if (ret)
    {
        ...
        if (ret)
        {
            ...
            if (ret)
            {
                ...
                if (!ret)
                    ERR("CryptVerifySignatureW failed\n");
            }
            else
                ERR("CryptHashData failed\n");
