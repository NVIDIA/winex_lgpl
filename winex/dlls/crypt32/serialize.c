 ...
 BYTE *pbElement, DWORD *pcbElement)
{
    ...
    if (context)
    {
        ...
        else
        {
                    ...
                    if (ret)
                    {
                        ...
                        if (buf)
                        {
                            ...
                            if (ret)
                            {
                                ...
                                totalSize += sizeof(WINE_CERT_PROP_HEADER) + propSize;
                            }
                        }
                        else
                            ret = FALSE;
            ...
            if (pcbElement != NULL)
                *pcbElement = totalSize + sizeof(WINE_CERT_PROP_HEADER) + cbEncodedContext;
        }
    }
    else
        ret = FALSE;
 ...
 const WINE_CERT_PROP_HEADER *hdr, const BYTE *pbElement, DWORD cbElement)
{
    ...
    else if (hdr->propID != CERT_CERT_PROP_ID && hdr->propID != CERT_FIRST_RESERVED_PROP_ID &&
     hdr->propID != CERT_CRL_PROP_ID && hdr->propID != CERT_CTL_PROP_ID)
    {
        ...
        case CERT_KEY_CONTEXT_PROP_ID:
            /* fall through */
        default:
            FIXME("add support for property %d\n", hdr->propID);
            ret = FALSE;
        }
    }
    else
