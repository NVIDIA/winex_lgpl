...
static void CRYPT_PrintChainElement(PCERT_CHAIN_ELEMENT element)
{
    TRACE_(chain)("    cbSize {%d} TrustStatus {0x%08x, 0x%08x}\n", element->cbSize, element->TrustStatus.dwErrorStatus, element->TrustStatus.dwInfoStatus);
    TRACE_(chain)("    pCertContext {%p} pRevocationInfo {%p}, pIssuanceUsage {%p}\n", element->pCertContext, element->pRevocationInfo, element->pIssuanceUsage);
    TRACE_(chain)("    pApplicationUsage {%p} pwszExtendedErrorInfo {%s}\n", element->pApplicationUsage, debugstr_w(element->pwszExtendedErrorInfo));
}

static void CRYPT_PrintChain(PCERT_SIMPLE_CHAIN chain)
{
    DWORD i;
    if (!chain)
    {
    ...
    TRACE_(chain)("cbSize {%d} TrustStatus {0x%08x, 0x%08x}\n", chain->cbSize, chain->TrustStatus.dwErrorStatus, chain->TrustStatus.dwInfoStatus);
    TRACE_(chain)("pTrustListInfo {%p} fHasRevocationFreshnessTime {%d} dwRevocationFreshnessTime {0x%x}\n", chain->pTrustListInfo, chain->fHasRevocationFreshnessTime, chain->dwRevocationFreshnessTime);
    TRACE_(chain)("cElement {%d}\n", chain->cElement);
    for (i = 0; i < chain->cElement; i++)
    {
        TRACE_(chain)("  element {%d}\n", i);
        CRYPT_PrintChainElement(chain->rgpElement[i]);
    }
}
 ...
 PCERT_SIMPLE_CHAIN chain, PCCERT_CONTEXT cert, DWORD subjectInfoStatus)
{
    ...
    TRACE_(chain)("(%p) (%p), (%p), (%d))\n", engine, chain, cert, subjectInfoStatus);

    element = CryptMemAlloc(sizeof(CERT_CHAIN_ELEMENT));
    if (TRACE_ON(chain))
        CRYPT_PrintChain(chain);
...
static void CRYPT_CheckRootCert(PCertificateChainEngine engine,
 PCERT_CHAIN_ELEMENT rootElement)
{
     ...
     CRYPT_VERIFY_CERT_SIGN_ISSUER_CERT, (void *)root, 0, NULL))
    {
        if (ERR_ON(crypt))
        {
            char certSHA1[256];
            BYTE sha1Value[20];
            DWORD sha1Size = sizeof(sha1Value);
            PCERT_SIGNED_CONTENT_INFO signedCert = NULL;
            DWORD size = 0;

            memset(sha1Value, 0, sha1Size);
            CertGetCertificateContextProperty(root, CERT_SHA1_HASH_PROP_ID, &sha1Value, &sha1Size);
            snprintf(certSHA1, sizeof(certSHA1),
                     "%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x",
                     sha1Value[ 0],sha1Value[ 1],sha1Value[ 2],sha1Value[ 3],sha1Value[ 4],
                     sha1Value[ 5],sha1Value[ 6],sha1Value[ 7],sha1Value[ 8],sha1Value[ 9],
                     sha1Value[10],sha1Value[11],sha1Value[12],sha1Value[13],sha1Value[14],
                     sha1Value[15],sha1Value[16],sha1Value[17],sha1Value[18],sha1Value[19]);
            CryptDecodeObjectEx(root->dwCertEncodingType, X509_CERT, root->pbCertEncoded, root->cbCertEncoded, CRYPT_DECODE_ALLOC_FLAG | CRYPT_DECODE_NOCOPY_FLAG, NULL, &signedCert, &size);
 
            ERR_(chain)("Last certificate's signature is invalid on fingerprint %s for oid %s\n", certSHA1, signedCert?signedCert->SignatureAlgorithm.pszObjId:"NULL"); 
        }
        rootElement->TrustStatus.dwErrorStatus |=
 ...
 PCERT_SIMPLE_CHAIN chain, LPFILETIME time)
{
    ...
    if (CRYPT_IsCertificateSelfSigned(rootElement->pCertContext))
    {
        ...
        CRYPT_CheckRootCert(engine, rootElement);
    }
    ...
    *infoStatus = 0;
     ...
     subject->pCertInfo->cExtension, subject->pCertInfo->rgExtension)))
    {
        ...
        if (ret)
        {
            ...
            TRACE_(chain)("info: {%d %p} {%d %p} {%d %p}\n", info->KeyId.cbData, info->KeyId.pbData,
                info->CertIssuer.cbData, info->CertIssuer.pbData,
                info->CertSerialNumber.cbData, info->CertSerialNumber.pbData);
 ...
 HCERTSTORE world, PCERT_SIMPLE_CHAIN chain)
{
     ...
     !CRYPT_IsCertificateSelfSigned(cert))
    {
        ...
        else
        {
            if (ERR_ON(chain))
            {
                static const int s_certNameSize = 256;
                char certName[s_certNameSize];
                PCERT_EXTENSION ext;

                certName[0] = 0;
                CertGetNameStringA(cert, CERT_NAME_RDN_TYPE, 0, NULL, certName, s_certNameSize);
                certName[s_certNameSize - 1] = 0;

                if ((ext = CertFindExtension(szOID_AUTHORITY_KEY_IDENTIFIER2, cert->pCertInfo->cExtension, cert->pCertInfo->rgExtension)))
                {
                    CERT_AUTHORITY_KEY_ID2_INFO *info;
                    char certSHA1[s_certNameSize];
                    DWORD size;
                    LPBYTE buf;

                    if (CryptDecodeObjectEx(cert->dwCertEncodingType, X509_AUTHORITY_KEY_ID2, ext->Value.pbData, ext->Value.cbData, CRYPT_DECODE_ALLOC_FLAG | CRYPT_DECODE_NOCOPY_FLAG, NULL, &info, &size) && size > 20)
                    {
                        buf = (LPBYTE)info->KeyId.pbData;
                        ...
                        ERR_(chain)("Couldn't find issuer with subject ID %s, for certificate %s. Halting chain creation\n", certSHA1, certName);
                    }
                    else
                        ERR_(chain)("Couldn't find issuer, for certificate %s. Halting chain creation\n", certName);
                }
                else
                    ERR_(chain)("Couldn't find issuer, for certificate %s. Halting chain creation\n", certName);
            }
 ...
 PCCERT_CHAIN_CONTEXT* ppChainContext)
{
    ...
    engine = (PCertificateChainEngine)hChainEngine;
