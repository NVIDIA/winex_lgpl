 ...
 PCERT_CHAIN_ELEMENT rootElement)
{
     ...
     CRYPT_VERIFY_CERT_SIGN_ISSUER_CERT, (void *)root, 0, NULL))
    {
        if (ERR_ON(crypt))
        {
            ...
            ERR("Last certificate's signature is invalid on fingerprint %s for oid %s\n", certSHA1, signedCert?signedCert->SignatureAlgorithm.pszObjId:"NULL");
        }
 ...
 PCCERT_CONTEXT prevIssuer, DWORD *infoStatus)
{
    ...
    *infoStatus = 0;
     ...
     subject->pCertInfo->cExtension, subject->pCertInfo->rgExtension)))
    {
        ...
        if (ret)
        {
            ...
            TRACE_(chain)("info: {%d %p} {%d %p} {%d %p}\n", info->KeyId.cbData, info->KeyId.pbData,
                info->AuthorityCertIssuer.cAltEntry, info->AuthorityCertIssuer.rgAltEntry,
                info->AuthorityCertSerialNumber.cbData, info->AuthorityCertSerialNumber.pbData);
    ...
    /* make sure to release the <prevIssuer> certificate context if we didn't go through a path
       that made a call to CertFindCertificateInStore() (which implicitly frees it).  This will
       make sure that we don't leak any certificate objects. */
    if (prevIssuer != NULL)
        CertFreeCertificateContext(prevIssuer);
    return issuer;
}
     ...
     !CRYPT_IsCertificateSelfSigned(cert))
    {
        ...
        else
        {
            if (ERR_ON(crypt))
            {
                ...
                ERR("For certificate {%s}\n", certName);
            }
            ERR("Couldn't find issuer, halting chain creation\n");
 ...
 PCCERT_CHAIN_CONTEXT* ppChainContext)
{
    ...
    {
        ERR("Invalid size! Got %u, expected %zu or %zu\n", pChainPara->cbSize, sizeof(CERT_CHAIN_PARA), sizeof(CERT_CHAIN_PARA_NO_EXTRA_FIELDS));
        SetLastError(E_INVALIDARG);
        return FALSE;
    }
    if (!hChainEngine)
