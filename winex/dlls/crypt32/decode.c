...
/* Given a PKCS#12-compliant data blob and a password, extract the certificate
 * and add it to the returned store.
 */
 ...
 DWORD dwFlags)
{
    ...
    FIXME_(crypt)("flag not handled: {0x%08x}\n", dwFlags);
    if ((ret = OPENSSL_loadOpenSSL()))
    {
        switch (ret)
        {
    ...
    if (szPassword)
    {
        ...
        len = WideCharToMultiByte(CP_UNIXCP, 0, szPassword, -1, NULL, 0, NULL, NULL);
        password = HeapAlloc(GetProcessHeap(), 0, len);
        if (!password)
        {
            ERR_(crypt)("password: could not allocate {%d} bytes\n", len);
            SetLastError(ERROR_NOT_ENOUGH_MEMORY);
            return NULL;
        }
        WideCharToMultiByte(CP_UNIXCP, 0, szPassword, -1, password, len, NULL, NULL);
    }

    /* convert blob data to pkcs12 struct */
    bio = BIO_new_mem_buf(pPFX->pbData, pPFX->cbData);
    if (!bio)
    {
        ERR_(crypt)("bio: could not allocate {%d} bytes\n", pPFX->cbData);
        SetLastError(ERROR_NOT_ENOUGH_MEMORY);
        if (password)
    ...
    if (!pkcs12)
    {
        ERR_(crypt)("could not read pkcs12 from bio\n");
        if (password)
            HeapFree(GetProcessHeap(), 0, password);
    ...
    ret = PKCS12_verify_mac(pkcs12, password, len);
    if (ret == 0)
        ERR_(crypt)("failed to verify pkcs12 {%p} with password \"%s\" using func {%p}\n", pkcs12, password, PKCS12_verify_mac);
    else
        TRACE_(crypt)("verified pkcs12 {%p} with password \"%s\" using func {%p}\n", pkcs12, password, PKCS12_verify_mac);

    /* extract private key and certificate */
    ret = PKCS12_parse(pkcs12, password, &pkey, &cert, NULL);
    TRACE_(crypt)("decoded pkcs12 {%p} resulted in pkey {%p} and cert {%p} returned {%d}\n", pkcs12, pkey, cert, ret);
    if (password)
        HeapFree(GetProcessHeap(), 0, password);
    PKCS12_free(pkcs12);
    if (ret)
    {
        ERR_(crypt)("failed to decode pkcs12 blob\n");
        SetLastError(CRYPT_E_BAD_ENCODE);
    }
    ...
    if (len < 0)
    {
        ERR_(crypt)("FIXME: negative length for buffer {%d}. bailing out.\n", len);
        SetLastError(CRYPT_E_BAD_MSG);
        return NULL;
    }
    TRACE_(crypt)("created buffer {%p} with length {%d}\n", buf, len);

    /* create store and appropriately-formatted certificate */
    certContext = CertCreateCertificateContext(X509_ASN_ENCODING | PKCS_7_ASN_ENCODING, buf, len);
    CRYPTO_free(buf);
    if (certContext)
    {
        const WCHAR storeName[] = {'S', 't', 'o', 'r', 'e', ' ', 'N', 'a', 'm', 'e', 0};
        store = CertOpenStore(CERT_STORE_PROV_MEMORY, 0, (HCRYPTPROV)NULL, CERT_STORE_CREATE_NEW_FLAG, storeName);
        if (store)
        {
            TRACE_(crypt)("created store {%p} and certContext {%p}\n", store, certContext);
            if (!CertAddCertificateContextToStore(store, certContext, CERT_STORE_ADD_ALWAYS, NULL))
            {
                ERR_(crypt)("unable to add certificate context to store\n");
        ...
        {
            ERR_(crypt)("unable to open memory store\n");
        }
    }
    else
    {
