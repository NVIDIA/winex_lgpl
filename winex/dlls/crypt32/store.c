 ...
 const void* pvPara)
{
    ...
    CRYPT_DATA_BLOB storeDebugNameBlob;
    LPCSTR storeDebugName = lpszStoreProvider;
#endif

    TRACE("(%s, %08x, %08tx, %08x, %p)\n", debugstr_a(lpszStoreProvider),
...
#ifdef WRITE_CERTS_TO_PDRIVE
        char temp[32];
        snprintf(temp, sizeof(temp), "INTOID-%02x", LOWORD(lpszStoreProvider));
        storeDebugName = temp;
#endif
...
#ifdef WRITE_CERTS_TO_PDRIVE
    storeDebugNameBlob.cbData = (strlen(storeDebugName)+1);
    storeDebugNameBlob.pbData = (BYTE*)storeDebugName;
    CertSetStoreProperty((HCERTSTORE)hcs, CERT_STORE_DEBUG_NAME_PROP_ID, 0, &storeDebugNameBlob);
#endif
 ...
 PCCERT_CONTEXT *ppStoreContext)
{
    ...
    if (toAdd)
    {
        if (store)
        {
            CERT_CONTEXT *newCert = NULL;

            /* certificate should add a copy of the certificate into the store */
            newCert = (CERT_CONTEXT*)CertCreateCertificateContext(toAdd->dwCertEncodingType, toAdd->pbCertEncoded, toAdd->cbCertEncoded);
            if (newCert)
            {
                newCert->hCertStore = hCertStore;
                TRACE("added cert %p to store %p\n", pCertContext, store);
            }
            else
                ERR("Failed to create new certificate\n");
            
            ret = store->certs.addContext(store, (void *)(newCert?newCert:toAdd),
...
{
    WINECRYPT_CERTSTORE *hcs = hCertStore;
    WINE_MEMSTORE *ms = (WINE_MEMSTORE *)hCertStore;
    ...
    if (ms && ms->certs && (dwFlags & CERT_CLOSE_STORE_CHECK_FLAG))
    {
        BOOL ret;

        ret = !ContextList_CertificatesInUse(ms->certs, 1);
        if (!ret)
            SetLastError(CRYPT_E_PENDING_CLOSE);

        return ret;
    }

    if ((hcs->dwOpenFlags & CERT_STORE_DEFER_CLOSE_UNTIL_LAST_FREE_FLAG))
    {
        if (ms && ContextList_CertificatesInUse(ms->certs, 1) && !(dwFlags & CERT_CLOSE_STORE_FORCE_FLAG))
        {
            TRACE("not closing due to open certificates\n");
            hcs->dwFlags |= CERTSTORE_FLAGS_DEFERING_CLOSE;
            return TRUE;
        }
