...
{
  InitializeCriticalSection( &IMalloc32_SpyCS );
  CRITICAL_SECTION_NAME( &IMalloc32_SpyCS, "IMalloc32_SpyCS" );
}

void destroy_malloc_cs(void)
{
  DeleteCriticalSection( &IMalloc32_SpyCS );
}
...
{
        Malloc32.SpyedBlocks = (LPVOID*)LocalReAlloc((HLOCAL)Malloc32.SpyedBlocks, NewLength * sizeof(LPVOID *), GMEM_ZEROINIT);
...
static int AddMemoryLocation(LPVOID * pMem)
{
	...
	if (!Malloc32.SpyedBlockTableLength) {
            if (!SetSpyedBlockTableLength(0x400)) return 0;
	}
	...
	while (*Current) {
	    ...
	    if (Current >= Malloc32.SpyedBlocks + Malloc32.SpyedBlockTableLength) {
	        ...
	        if (!SetSpyedBlockTableLength( Malloc32.SpyedBlockTableLength + 0x400 )) return 0;
	    }
	};
