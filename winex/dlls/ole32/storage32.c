 ...
 * The big block chain will be destroyed.
 */
SmallBlockChainStream* Storage32Impl_BigBlocksToSmallBlocks(
                      StorageImpl* This,
                      BlockChainStream** ppbChain)
{
  ULONG sbHeadOfChain = BLOCK_END_OF_CHAIN;
  ULARGE_INTEGER size, offset;
  ULONG cbRead, cbWritten, cbTotalRead, cbTotalWritten;
  ULONG propertyIndex;
  ...
  StgProperty chainProperty;
  BYTE *buffer;
  SmallBlockChainStream *sbTempChain = NULL;
  SmallBlockChainStream *smallBlockChain = NULL;

  TRACE("This: %p, BigBlockChain: %p\n", This, *ppbChain);

  /*
   * Create a temporary small block chain that doesn't have
   * an associated property. This temporary chain will be
   * used to copy data from big blocks to small blocks.
   */
  sbTempChain = SmallBlockChainStream_Construct(This,
                                                &sbHeadOfChain,
                                                PROPERTY_NULL);

  /*
   * Grow the small block chain.
   */
  size = BlockChainStream_GetSize(*ppbChain);
  SmallBlockChainStream_SetSize(sbTempChain, size);

  /*
   * Copy the contents of the big block chain to the small block chain
   * by big block size increments.
   */
  offset.s.LowPart = 0;
  offset.s.HighPart = 0;
  cbTotalRead = 0;
  ...
  buffer = (BYTE *) HeapAlloc(GetProcessHeap(),0,DEF_BIG_BLOCK_SIZE);
  do
  {
    ULONG amtToRead = min (size.s.LowPart - cbTotalWritten,
                           DEF_BIG_BLOCK_SIZE);

    cbRead = 0;
    BlockChainStream_ReadAt(*ppbChain, offset, amtToRead,
                            buffer, &cbRead);
    cbTotalRead += cbRead;
    ...
    if (cbRead > 0)
    {
        ...
        /* This returns failure if not all bytes are written */
        successWrite = SmallBlockChainStream_WriteAt(sbTempChain,
                                                     offset,
                                                     cbRead,
                                                     buffer,
                                                     &cbWritten);
        cbTotalWritten += cbWritten;
        offset.s.LowPart += cbWritten;
        if (!successWrite)
            ERR ("WriteAt failed. offset: %lu, cbRead: %lu, cbWritten: %lu\n",
                 offset.s.LowPart, cbRead, cbWritten);
    }

  } while ((cbRead > 0) && successWrite);
  HeapFree(GetProcessHeap(),0,buffer);

  assert(cbTotalRead == cbTotalWritten);

  /*
   * Destroy the big block chain.
   */
  propertyIndex = (*ppbChain)->ownerPropertyIndex;
  size.s.HighPart = 0;
  size.s.LowPart  = 0;
  BlockChainStream_SetSize(*ppbChain, size);
  BlockChainStream_Destroy(*ppbChain);
  *ppbChain = 0;

  /*
   * Change the property information. This chain is now a small block chain
   * and it doesn't reside in the big blocks chain anymore.
   */
  StorageImpl_ReadProperty(This, propertyIndex, &chainProperty);
  ...
  /*
   * Destroy the temporary propertyless small block chain.
   * Create the new small chain associated with this property.
   */
  SmallBlockChainStream_Destroy(sbTempChain);
  smallBlockChain = SmallBlockChainStream_Construct(This,
                                                    NULL,
                                                    propertyIndex);

  return smallBlockChain;
}
  ...
  SmallBlockChainStream* This)
{
  ...
  if (This->headOfStreamPlaceHolder)
      return *(This->headOfStreamPlaceHolder);
  else if (This->ownerPropertyIndex)
  {
    readSuccessful = StorageImpl_ReadProperty(
                      This->parentStorage,
                      This->ownerPropertyIndex,
...
ULARGE_INTEGER SmallBlockChainStream_GetSize(SmallBlockChainStream* This)
{
  ...
  else
  {
      ...
      /* This is a temporary chain - return upper bound of its size */
      result.s.HighPart = 0;
      result.s.LowPart = SmallBlockChainStream_GetCount (This) *
