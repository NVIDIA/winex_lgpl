...
HRESULT WINAPI EnableThemeDialogTexture(HWND hwnd, DWORD dwFlags)
{
    ...
    TRACE("(%p,0x%08x\n", hwnd, dwFlags);
    res = SetPropW (hwnd, MAKEINTATOMW(atDialogThemeEnabled), 
...
BOOL WINAPI IsThemeDialogTextureEnabled(HWND hwnd)
{
    ...
    dwDialogTextureFlags = HandleToULong( GetPropW( hwnd, MAKEINTATOMW(atDialogThemeEnabled) ));
 ...
 */
HRESULT WINAPI DrawThemeParentBackground(HWND hwnd, HDC hdc, const RECT *prc)
{
    RECT rt;
    POINT org;
    HWND hParent;
...
static HRESULT get_image_part_size (HTHEME hTheme, HDC hdc, int iPartId,
                                    int iStateId, const RECT *prc, enum THEMESIZE eSize,
                                    POINT *psz)
{
    HRESULT hr = S_OK;
    HBITMAP bmpSrc;
...
static HRESULT get_border_background_size (HTHEME hTheme, int iPartId,
                                           int iStateId, enum THEMESIZE eSize, POINT* psz)
{
    HRESULT hr = S_OK;
    int bordersize = 1;
