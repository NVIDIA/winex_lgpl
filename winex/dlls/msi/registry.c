...
/* Wrapper around RegSetValueExW */
LONG msi_reg_set_val( HKEY hkey, LPCWSTR name, DWORD reserved, DWORD type, 
                      const BYTE *lpData, DWORD cbData )
{
    if (type == REG_SZ)
        return msi_reg_set_val_str(hkey,name,(LPCWSTR)lpData);
    else if (type == REG_EXPAND_SZ)
        return msi_reg_set_val_expand_str(hkey,name,(LPCWSTR)lpData);
    else if (type == REG_MULTI_SZ)
        return msi_reg_set_val_multi_str(hkey,name,(LPCWSTR)lpData);
    else
        return RegSetValueExW( hkey, name, reserved, type, lpData, cbData );
}
...
LONG msi_reg_set_val_expand_str( HKEY hkey, LPCWSTR name, LPCWSTR value )
{
    static const WCHAR emptyW[] = {0};
    DWORD len;
    if (!value) value = emptyW;
    len = (lstrlenW(value) + 1) * sizeof(WCHAR);
    return RegSetValueExW( hkey, name, 0, REG_EXPAND_SZ, (const BYTE *)value, len );
}
