                                      ...
                                      LPCWSTR usersid, LPCWSTR prodcode)
{
    ...
    /* While this may appear to be overkill, there's no guarantee that some
       of our other arguments aren't NULL, in which case we need to
       allocate enough memory here because a NULL pointer formats
       to "(null)" not "" and lstrlenW(NULL) == 0.

       We shouldn't probably get NULL passed in here, but that's
       another bug for another time in another location. 
    */
