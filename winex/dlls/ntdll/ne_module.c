...
void NE_DumpModule( HMODULE16 hModule )
{
    ...
    pstr = (BYTE *)pModule + pModule->name_table;
    while (*pstr)
    {
        DPRINTF( "%*.*s: %d\n", *pstr, *pstr, pstr + 1,
	      *(WORD *)(pstr + *pstr + 1) );
    ...
    {
        pstr = (BYTE *)GlobalLock16( pModule->nrname_handle );
        while (*pstr)
        {
            DPRINTF( "%*.*s: %d\n", *pstr, *pstr, pstr + 1,
                   *(WORD *)(pstr + *pstr + 1) );
...
WORD NE_GetOrdinal( HMODULE16 hModule, const char *name )
{
    ...
    cpnt = (BYTE *)pModule + pModule->name_table;
    ...
    cpnt = (BYTE *)GlobalLock16( pModule->nrname_handle );
 ...
 */
static
BOOL16 NE_FindEntryPoint( HMODULE16 hModule, WORD ordinal, ET_ENTRY** ppEntry, NE_MODULE** ppModule )
{
    NE_MODULE *pModule;
    ET_BUNDLE *bundle;
    ET_ENTRY* pEntry;
    ...
    pEntry = (ET_ENTRY *)((BYTE *)(bundle + 1));

    pEntry += ordinal - bundle->first - 1;
...
FARPROC16 NE_GetEntryPointEx( HMODULE16 hModule, WORD ordinal, BOOL16 snoop )
{
    ...
    if( !NE_FindEntryPoint( hModule, ordinal, &entry, &pModule ) )
      return NULL;
...
HANDLE NE_OpenFile( NE_MODULE *pModule )
{
    ...
    name = (char *)NE_MODULE_NAME( pModule );
...
#define READ(offset,size,buffer) \
    ...
    ofs->cBytes = size - 1;
    ofs->fFixedDisk = 1;
    strcpy( (char *)ofs->szPathName, path );
    pData += size;
...
static HINSTANCE16 NE_LoadModule( LPCSTR name, BOOL lib_only )
{
    ...
    hModule = NE_LoadExeHeader( DosFileHandleToWin32Handle(hFile),
                                (char *)ofs.szPathName );
    _lclose16( hFile );
...
DWORD NE_StartTask(void)
{
    ...
    if ( hInstance >= 32 )
    {
        ...
        FreeSelector16( SELECTOROF(pTask->teb->WOW32Reserved) );
        ...
        pTask->teb->WOW32Reserved = (void *)(MAKESEGPTR( GlobalHandleToSel16(hInstance), sp ));
        ...
        TRACE("Starting main program: cs:ip=%04lx:%04lx ds=%04lx ss:sp=%04x:%04x\n",
              ...
              SELECTOROF(pTask->teb->WOW32Reserved),
              OFFSETOF(pTask->teb->WOW32Reserved) );
                                  ...
                                  INT16 nSize )
{
    ...
    if (!(pModule = NE_GetPtr( hModule ))) return 0;
    lstrcpynA( lpFileName, (char *)NE_MODULE_NAME(pModule), nSize );
    if (pModule->expected_version >= 0x400)
        GetLongPathNameA((char *)NE_MODULE_NAME(pModule), lpFileName, nSize);
    TRACE("%04x -> '%s'\n", hModule, lpFileName );
...
HMODULE16 WINAPI GetModuleHandle16( LPCSTR name )
{
	...
	pModule = NE_GetPtr( hModule );
        ...
        name_table = (char *)pModule + pModule->name_table;
	...
	if (!pModule->fileinfo) continue;
        ...
        loadedfn = (char*)ofs->szPathName + strlen((char *)ofs->szPathName);
...
BOOL16 WINAPI ModuleNext16( MODULEENTRY *lpme )
{
    ...
    lstrcpynA( lpme->szExePath, (char *)NE_MODULE_NAME(pModule),
               sizeof(lpme->szExePath) );
