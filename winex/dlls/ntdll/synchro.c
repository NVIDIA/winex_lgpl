...
static void call_apcs( BOOL alertable )
{
    ...
    for (;;)
    {
        ...
        switch(type)
        {
			...
			case APC_ASYNC:
			{
				typedef VOID (CALLBACK *AsyncCallback_t)(LPVOID, LPVOID);
				AsyncCallback_t pAsyncProc = (AsyncCallback_t)proc;

				pAsyncProc( args[0], args[1]);
				break;
			}
			case APC_USER:
			{
				typedef VOID (CALLBACK *UserCallback_t)(LPVOID);
				UserCallback_t pUserProc = (UserCallback_t)proc;
			...
			case APC_TIMER:
			{
				PTIMERAPCROUTINE pTimerProc = (PTIMERAPCROUTINE)proc;

				/* convert sec/usec to NT time */
				DOSFS_UnixTimeToFileTime( (time_t)args[0], &ft, (DWORD)args[1] * 10 );
				pTimerProc( args[2], ft.dwLowDateTime, ft.dwHighDateTime );
				break;
			}
			case APC_ASYNC_IO:
