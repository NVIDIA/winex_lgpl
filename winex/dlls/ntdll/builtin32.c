...
static void load_library( void *base, const char *filename )
{
    ...
    /* builtin dlls "exist" in the system directory */
    if (!FILE_contains_path(filename) &&
    ...
    if (!(wm = PE_CreateModule( module, path, 0, 0, TRUE )))
    {
        ERR( "can't load %s\n", path );
...
{
    WINE_MODREF   *wm;
    /* FIXME: dllname should allocated on the heap or something. */
    char dllname[MAX_PATH+1], *p;
    LPCSTR name;
    void *handle;
    ...
    if (strlen(name) >= sizeof(dllname)-4)
    {
        ERR("static buffer is too small for this dll name. please make me dynamic\n");
        goto error;
    }
