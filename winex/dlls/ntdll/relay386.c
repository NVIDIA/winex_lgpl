...
static void get_entry_point( char *buffer, DEBUG_ENTRY_POINT *relay )
{
    ...
    for (wm = MODULE_modref_list; wm; wm = wm->next)
    {
        ...
        base = (char *)wm->module;
        dir = &PE_HEADER(base)->OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_EXPORT];
...
void RELAY_SetupDLL( const char *module )
{
    ...
    dir = &PE_HEADER(module)->OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_EXPORT];
    if (!dir->Size) return;
