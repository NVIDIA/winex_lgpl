...
static TDB *TASK_Create( NE_MODULE *pModule, UINT16 cmdShow, TEB *teb, LPCSTR cmdline, BYTE len )
{
    ...
    strcpy( (char *)pTask->curdir, "\\" );
    lstrcpynA( (char *)pTask->curdir + 1,
    ...
    strncpy( (char *)pTask->module_name, name, sizeof(pTask->module_name) );
...
void WINAPI InitTask16( CONTEXT86 *context )
{
    ...
    pinstance->stackmin    = OFFSETOF( pTask->teb->WOW32Reserved ) + sizeof( STACK16FRAME );
...
HQUEUE16 WINAPI GetThreadQueue16( DWORD thread )
{
    ...
    {
        ERR( "Breaking 16 bit for tid %ld\n", thread );
        teb = THREAD_IdToTEB( thread );
    }
#endif
...
void WINAPI SwitchStackTo16( WORD seg, WORD ptr, WORD top )
{
    ...
    TRACE("old=%04x:%04x new=%04x:%04x\n",
          ...
          OFFSETOF( pTask->teb->WOW32Reserved ), seg, ptr );

    /* Save the old stack */

    oldFrame = THREAD_STACK16( pTask->teb );
    /* pop frame + args and push bp */
    pData->old_ss_sp   = ((DWORD)pTask->teb->WOW32Reserved) + sizeof(STACK16FRAME)
                           + 2 * sizeof(WORD);
    *(WORD *)MapSL(pData->old_ss_sp) = oldFrame->bp;
    ...
    pTask->teb->WOW32Reserved = (void *)(MAKESEGPTR( seg, ptr - copySize ));
...
BOOL16 WINAPI TaskNext16( TASKENTRY *lpte )
{
    ...
    lstrcpynA( lpte->szModule, (char *)pTask->module_name,
               sizeof(lpte->szModule) );
