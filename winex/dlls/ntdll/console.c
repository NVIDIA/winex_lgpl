...
static	BOOL	start_console_renderer(void)
{
	...
	else if ( errno == EINVAL ) { /* full_argv0 is not a symlink. Not an error. */ }
...
void create_console_cs(void)
{
    InitializeCriticalSection( &CONSOLE_CritSect );
    CRITICAL_SECTION_NAME( &CONSOLE_CritSect, "console_ctrl_section" );
}
...
int     CONSOLE_HandleCtrlC(void)
{
    ...
    __EXCEPT(CONSOLE_CtrlEventHandler)
    {
         ...
         */
        HANDLE hnd;
        if ((hnd = CreateThread(NULL, 0, CONSOLE_HandleCtrlCEntry, (void*)CTRL_C_EVENT, 0, NULL)) != 0)
            CloseHandle(hnd);
    }
    __ENDTRY;
    return 1;
				     ...
				     DWORD dwProcessGroupID)
{
    ...
    SERVER_START_REQ( send_console_signal )
    {
        ...
        req->group_id = (process_id_t)dwProcessGroupID;
        ret = !wine_server_call_err( req );
    }
    SERVER_END_REQ;
