 ...
 */
inline static void set_registry_keys( HKEY hkey, char *module,
                                      const BYTE *buffer )
{
    static int warn;
    ...
    while (p)
    {
        ...
        if (RegQueryValueExA( hkey, p, 0, NULL, NULL, NULL ) == ERROR_FILE_NOT_FOUND)
            RegSetValueExA( hkey, p, 0, REG_SZ, buffer,
...
static BOOL get_app_load_order( const char *module, enum loadorder_type lo[], BOOL *got_default )
{
    ...
    count = sizeof(buffer);
    if ((res = RegQueryValueExA( appkey, module, NULL, &type, (BYTE *)buffer,
                                 &count )))
    {
        if (!(res = RegQueryValueExA( appkey, "*", NULL, &type,
                                      (BYTE *)buffer, &count )))
            *got_default = TRUE;
    }
                                     ...
                                     BOOL *got_default )
{
    ...
    RegCloseKey( hkey );
    if (res) return FALSE;
    return ParseLoadOrder( (char *)buffer, lo );
}
...
static void get_default_load_order( enum loadorder_type lo[] )
{
    ...
    if (!loaded)
    {
                                 ...
                                 "Software\\Wine\\Wine\\Config\\DllDefaults", &hkey )))
        {
            ...
            res = RegQueryValueExA( hkey, "DefaultLoadOrder", NULL, &type,
                                    (BYTE *)buffer, &count );
            RegCloseKey( hkey );
        }
