	...
	PACE_HEADER* x)
{
	...
	*x = 0;
	...
	for (i=0;i<acl->AceCount;i++) {
		if ((DWORD)ace>=(((DWORD)acl)+acl->AclSize)) break;
		ace = (PACE_HEADER)(((BYTE*)ace)+ace->AceSize);
	}

	/* x should be non null if ACL is valid but there isn't a free ace */
	*x = ace;
	...
	DWORD acelen)
{
	...
	{
		if( targetace == NULL ) return STATUS_INVALID_ACL;
	...
	if ((DWORD)targetace+acelen>(DWORD)acl+acl->AclSize) /* too many aces */
	...
	IN PSID pSid)
{
	...
	if( !RtlFirstFreeAce( pAcl, (PACE_HEADER*)&lpAAA) )
	{
		if( lpAAA == NULL ) return STATUS_INVALID_ACL;
		else return STATUS_ALLOTTED_SPACE_EXCEEDED;
	}

	if( ((BYTE*)lpAAA + sizeof( *lpAAA ) + dwSidLength ) > ((BYTE*)pAcl + pAcl->AclSize) )
	{
		return STATUS_ALLOTTED_SPACE_EXCEEDED;
	}

	lpAAA->Header.AceType = ACCESS_ALLOWED_ACE_TYPE;
	lpAAA->Header.AceFlags = 0;
	lpAAA->Header.AceSize = sizeof( *lpAAA );
	lpAAA->Mask = AccessMask;
	memcpy( &lpAAA->SidStart, pSid, dwSidLength );
       ...
       BOOLEAN AllocateString)
{
	...
	if (AllocateString)
	{
	  String->Buffer = RtlAllocateHeap(GetProcessHeap(), 0, Length + sizeof(WCHAR));
	  if (String->Buffer == NULL) return STATUS_NO_MEMORY;
	...
	RtlAnsiStringToUnicodeString(String, &AnsiStr, TRUE);
