...
struct EFSS                     /* FAT32 extended free space structure (from Ralf Brown's Interrupt List) */
{
    WORD  struct_size;  /* (ret) size of returned structure */
    WORD  struct_ver;   /* (call) structure version (0000h) / (ret) actual structure version (0000h) */
    DWORD sectors_per_cluster; /* number of sectors per cluster (with adjustment for compression) */
    DWORD bytes_per_sector;    /* number of bytes per sector */
    DWORD avail_clusters; /* number of available clusters */
    DWORD total_clusters; /* total number of clusters on the drive */
    DWORD avail_sectors;  /* number of physical sectors available on the drive, without adjustment for compression */
    DWORD total_sectors;  /* total number of physical sectors on the drive, without adjustment for compression */
    DWORD avail_alloc_units; /* number of available allocation units, without adjustment for compression */
    DWORD total_alloc_units; /* total allocation units, without adjustment for compression */
...
static BOOL INT21_CreateHeap(void)
{
    ...
    heap->InDosFlag = 0;
    memcpy(heap->biosdate, "01/01/80", 8);
...
static BOOL INT21_CreateTempFile( CONTEXT86 *context )
{
    ...
    for (;;)
    {
        sprintf( p, "wine%04x.%03d", (int)wine_gettid_or_pid(), counter );
...
static int INT21_FindNextFCB( CONTEXT86 *context )
{
	...
	*(BYTE *)pResult = 0xff;
	/* leave reserved field behind */
	pResult = (DOS_DIRENTRY_LAYOUT *)((BYTE *)pResult + 6);
	*(BYTE *)pResult = entry.dwFileAttributes;
	pResult = (DOS_DIRENTRY_LAYOUT *)((BYTE *)pResult + 1);
    }
...
void WINAPI DOS3Call( CONTEXT86 *context )
{
    ...
    case 0x73: /* MULTIPLEXED: Win95 OSR2/Win98 FAT32 calls */
		...
		case 0x03:	/* AX = 7303h: Get Extended free space on drive */
			 ...
			 *         CF set on error
			 *           AX = error code
			 * Based on info from Ralf Brown's Interrupt List 
			 */
			{
			  char *drive;
			  int buffer_len;
			  ...
			  /* FIXME: these should probably be detected somehow */
			  int sector_bytes = 512;   /* assume 512 bytes (pretty standard for harddrives) */
			  int cluster_sectors = 64; /* assume 64 sectors = 32K bytes */
			  int cluster_bytes = cluster_sectors * sector_bytes;

			  drive = (char *)CTX_SEG_OFF_TO_LIN(context,  context->SegDs, context->Edx);
			  efss = (struct EFSS *)CTX_SEG_OFF_TO_LIN(context, context->SegEs, context->Edi);
			  buffer_len = CX_reg(context);
			  TRACE("get extended free space on drive=%s, efss=%p, len=%d\n", 
			        drive, efss, buffer_len );

			  if( buffer_len != sizeof(struct EFSS) ) {
			    WARN("unexpected buffer length (%d!=%d)\n", buffer_len, sizeof(struct EFSS));
			    SET_CFLAG(context);
			    AX_reg(context) = 0x18;  /* bad buffer length */
			    break;
			  }
			  
			  /* it would be nice to just call GetDiskFreeSpaceA, and get the results in clusters and
			   * sectors as required, but that is limited to a max of 2GB, so here goes the hard way */
			  if( !GetDiskFreeSpaceExA(drive, NULL, &total, &free) ) {
			    SET_CFLAG(context);
			    AX_reg(context) = 0x15;  /* invalid drive */
			    break;
			  }

			  TRACE("totalbytes=%llu, freebytes=%llu\n", total.QuadPart, free.QuadPart);
			  efss->struct_size = sizeof(struct EFSS);
			  efss->struct_ver = 0;
			  efss->sectors_per_cluster = cluster_sectors;
			  efss->bytes_per_sector = sector_bytes;
			  efss->avail_clusters = free.QuadPart / cluster_bytes;
			  efss->total_clusters = total.QuadPart / cluster_bytes;
			  efss->avail_sectors = free.QuadPart / sector_bytes ;
			  efss->total_sectors = total.QuadPart / sector_bytes;
			  efss->avail_alloc_units = efss->avail_sectors;
			  efss->total_alloc_units = efss->total_sectors;
