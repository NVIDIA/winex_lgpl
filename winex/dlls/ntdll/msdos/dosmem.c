...
BYTE * DOSMEM_BiosSys()
{
    return (BYTE *)(DOSMEM_dosmem+0xf0000);
}
...
static void setup_dos_mem( int dos_init )
{
    ...
    switch (wine_mmap_is_reserved((void*)page_size, 0x110000-page_size))
    {
    case -1:
        ERR("dos memory is partially reserved\n");
        ExitProcess(1);
    case 0: /* not reserved */
    ...
    if (addr == (void *)page_size)  /* we got what we wanted */
    {
        ...
        if (wine_mmap_is_reserved(NULL, 0x110000) == -1)
        {
            ERR("dos memory is partially reserved\n");
            ExitProcess(1);
        } /* this should work if it's reserved or not: */
