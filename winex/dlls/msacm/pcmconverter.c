 ...
 *	+ sampling rate (8.0, 11.025, 22.05, 32.0, 44.1 kHz are defined, but 
 *	  algo shall work in all cases)
 *
...
static	LRESULT	PCM_StreamOpen(PACMDRVSTREAMINSTANCE adsi)
{
    ...
    TRACE("Source: Bits %d. nChannels %d. Rate %d\n", adsi->pwfxSrc->wBitsPerSample, adsi->pwfxSrc->nChannels, adsi->pwfxSrc->nSamplesPerSec);
    TRACE("Dest: Bits %d. nChannels %d. Rate %d\n", adsi->pwfxDst->wBitsPerSample, adsi->pwfxDst->nChannels, adsi->pwfxDst->nSamplesPerSec);

    if (PCM_GetFormatIndex(adsi->pwfxSrc) == 0xFFFFFFFF ||
	PCM_GetFormatIndex(adsi->pwfxDst) == 0xFFFFFFFF)
    {
	WARN("Impossible to open stream\n");
	return ACMERR_NOTPOSSIBLE;
    }
