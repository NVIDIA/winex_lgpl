                                      ...
                                      WPARAM wParam, LPARAM lParam )
{
    ...
    if (TRACE_ON(relay)) {
        if( TRACE_ON(timestamp) ) DPRINTF( "%ld - ", GetTickCount() );
        DPRINTF( "%04lx:Call(%u) window proc %p (hwnd=%08x,msg=%s,wp=%08x,lp=%08lx)\n",
                 GetCurrentThreadId(), (NtCurrentTeb()->uRelayLevel)++, proc, hwnd,
    ...
    if (TRACE_ON(relay)) {
        if( TRACE_ON(timestamp) ) DPRINTF( "%ld - ", GetTickCount() );
        DPRINTF( "%04lx:Ret (%u) window proc %p (hwnd=%08x,msg=%s,wp=%08x,lp=%08lx) retval=%08lx\n",
                 GetCurrentThreadId(), --(NtCurrentTeb()->uRelayLevel), proc, hwnd,
                 SPY_GetMsgName(msg, hwnd), wParam, lParam, retvalue );
    }
    return retvalue;
}
                                         ...
                                         LPARAM lParam )
{
    ...
    if( (unmap = WINPROC_MapMsg32ATo32W( hwnd, msg, &wParam, &lParam )) == -1) {
        ERR("Message translation failed. (msg=%s,wp=%08x,lp=%08lx)\n",
                       SPY_GetMsgName(msg, hwnd), wParam, lParam );
        return 0;
    }
