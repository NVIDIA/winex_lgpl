 ...
 */
void queue_hardware_message( MSG *msg, ULONG_PTR extra_info, enum message_type type )
{
    SERVER_START_REQ( send_message )
    {
        req->type   = type;
        req->id     = (thread_id_t)GetWindowThreadProcessId( msg->hwnd, NULL );
        req->win    = msg->hwnd;
        req->msg    = msg->message;
        ...
        (raw_message == WM_MBUTTONDOWN))
    {
        ...
        if (msg->hwnd != GetActiveWindow() && hwndTop != GetDesktopWindow())
        {
            ...
            case 0:
                ...
                {
                    if (!WINPOS_SetActiveWindow( hwndTop, TRUE, TRUE, TRUE ))
                        eatMsg = TRUE;
                }
                break;
            default:
                                       ...
                                       UINT first, UINT last, BOOL remove )
{
    ...
    {
        if (!process_raw_mouse_message( msg, extra_info, TRUE)) return FALSE;
    }
    else
    {
...
SHORT WINAPI GetKeyState(INT vkey)
{
    ...
    /* Windows is able to handle junk being passed and just ignores it. 'a' is
     * also not the same as 'A' so it should not be upper cased automatically. */
    if( vkey & 0xffffff00 ) return 0;
...
BOOL WINAPI TranslateMessage( const MSG *msg )
{
    ...
    /* the preferred method is to extract the queued char messages from the driver.  This is
       currently only supported on SDLDRV.  The char messages are generated and queued as
       keyboard events arrive.  This allows us to support dead character conversion since we 
       can use all the original mac event information instead of having to try to recreate
       it from the windows side. */
    if (USER_Driver.pGetCharMessages){
        INT         msgs[64];
        USHORT      chars[64];
        UINT        actualCount;
        INT         result;
        ...
        TRACE_(key)("calling user driver {msg = {hwnd = 0x%08x, message = 0x%04x, wParam = 0x%08x, lParam = 0x%08lx, time = 0x%08lx}}\n",
                    msg->hwnd, msg->message, msg->wParam, msg->lParam, msg->time);

        /* keep retrieving messages as long as we are filling the message buffers.  In theory,
           a single keystroke can produce up to 255 unicode characters.  In practicality, the
           actual result is rarely more than 4. */
        do{

            /* retrieve all the matching character messages from the driver */
            result = USER_Driver.pGetCharMessages(msg->time, msgs, chars, sizeof(msgs) / sizeof(msgs[0]), &actualCount);

            TRACE_(key)("retrieved %d messages {result = %d}\n", actualCount, result);


            /* send these messages off to the app */
            for (i = 0; i < actualCount; i++){
                TRACE_(key)("PostMessage(%s, 0x%08x, 0x%08lx)\n", SPY_GetMsgName(msgs[i], msg->hwnd), chars[i], msg->lParam);
                PostMessageW(msg->hwnd, msgs[i], chars[i], msg->lParam);
            }
        ...
        TRACE_(key)("trying to convert characters to unicode to generate char messages\n");
