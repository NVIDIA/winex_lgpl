...
BOOL WINAPI AdjustWindowRectEx( LPRECT rect, DWORD style, BOOL menu, DWORD exStyle )
{
    ...
    TRACE( "Adjusted values are: (%d,%d)-(%d,%d)\n",
             rect->left, rect->top, rect->right, rect->bottom );
    ...
    case HTCAPTION:
        {
            ...
            if( WINPOS_SetActiveWindow(top, TRUE, TRUE, TRUE) || (GetActiveWindow() == top) )
	...
	if (wParam == SC_ABOUTWINE)
        {
            ...
            if (hmodule)
            {
                typedef BOOL (WINAPI* ShellAboutA_t)(HWND,LPCSTR,LPCSTR,HICON);
                ShellAboutA_t aboutproc = (ShellAboutA_t)GetProcAddress( hmodule, "ShellAboutA" );
