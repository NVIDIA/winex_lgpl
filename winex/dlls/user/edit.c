...
static void EDIT_EM_ReplaceSel(HWND hwnd, EDITSTATE *es, BOOL can_undo, LPCWSTR lpsz_replace, BOOL send_update)
{
	...
	s = es ? es->selection_start : 0;
	e = es ? es->selection_end : 0;
	...
	if (e != s) {
		...
		if (can_undo) {
			...
			if (!es->undo_insert_count && (*es->undo_text && (s == es->undo_position))) {
				...
				lstrcpynW(es->undo_text + utl, es->text + s, e - s + 1);
				...
				lstrcpynW(es->undo_text, es->text + s, e - s + 1);
				es->undo_position = s;
			}
