...
static LPWINE_CLIPFORMAT CLIPBOARD_RenderText( UINT wFormat )
{
    ...
    TRACE("{source = %s (%u), target = %s (%u)}\n", 
            lpSource ? lpSource->Name : "<NULL>", 
            lpSource ? lpSource->wFormatID : 0,
            lpTarget ? lpTarget->Name : "<NULL>",
            lpTarget ? lpTarget->wFormatID : 0);
 ...
 */
static UINT CLIPBOARD_EnumClipboardFormats( UINT wFormat, BOOL checkFormatAvailable )
{
    LPWINE_CLIPFORMAT lpFormat = ClipFormats;
    BOOL bFormatPresent;
    ...
    else
    {
        ...
        /* native returns 0 if an enumeration is started at a format that does NOT have any data available */
        if (checkFormatAvailable && !USER_Driver.pIsClipboardFormatAvailable(wFormat))
            return 0;
...
HANDLE16 WINAPI SetClipboardData16( UINT16 wFormat, HANDLE16 hData )
{
    ...
    /* when this function is properly implemented it should probably be called here */
...
HANDLE WINAPI SetClipboardData( UINT wFormat, HANDLE hData )
{
    ...
    /* when this function is actually implemented it should probably be called here */
    USER_Driver.pSetClipboardData(wFormat);
...
HANDLE WINAPI GetClipboardData( UINT wFormat )
{
	...
	size = GlobalSize16(lpRender->hData16);
        ...
        lpRender->hData32 = GlobalAlloc(GMEM_ZEROINIT | GMEM_MOVEABLE | GMEM_DDESHARE, size);
...
UINT WINAPI EnumClipboardFormats( UINT wFormat )
{
    ...
    return CLIPBOARD_EnumClipboardFormats(wFormat, TRUE);
}
...
BOOL WINAPI IsClipboardFormatAvailable( UINT wFormat )
{
    ...
    {
        UINT iret = CLIPBOARD_EnumClipboardFormats(wFormat - 1, FALSE);
        if ((wFormat == CF_TEXT) || (wFormat == CF_OEMTEXT) || (wFormat == CF_UNICODETEXT))
