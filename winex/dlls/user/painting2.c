...
static void RDW_UpdateRgns( WND* wndPtr, HRGN hRgn, UINT flags, BOOL firstRecursLevel )
{
	...
	if( hRgn > 1 )
	{
	    switch( (UINT_PTR)wndPtr->hrgnUpdate )
	    {
		default:
 ...
 */
void WINAPI UpdateLayeredWindow( HWND hwnd, HDC hdcDst, POINT *pptDst, SIZE *psize,
                                 HDC hdcSrc, POINT *pptSrc, COLORREF crKey,
                                 BLENDFUNCTION *pblend, DWORD dwFlags )
{
    FIXME("(%04x %04x %p %p %04x %p %lx %p flags=%lx) stub\n", 
...
UINT16 WINAPI RealizePalette16( HDC16 hDC )
{
    ...
    if (realized && IsDCCurrentPalette16( hDC ))
    {
        ...
        HWND hWnd = WindowFromDC( hDC );
        if (hWnd) SendNotifyMessageA( HWND_BROADCAST, WM_PALETTECHANGED, (WPARAM)hWnd, 0L);
    }
    return realized;
}
