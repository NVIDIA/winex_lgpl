...
static WPARAM map_wparam_AtoW( UINT message, WPARAM wparam )
{
    if (message == WM_CHARTOITEM ||
        ...
        /* native doesn't seem to convert character messages when calling from any of
           the A or W versions of GetMessage(), PeekMessage(), DispatchMessage(), etc. */
        /*message == WM_CHAR ||
        message == WM_DEADCHAR ||
        message == WM_SYSCHAR ||
...
void create_dde_cs(void)
{
  InitializeCriticalSection( &dde_crst );
  CRITICAL_SECTION_NAME( &dde_crst, "Raw_DDE_CritSect" );
}

void destroy_dde_cs(void)
{
  DeleteCriticalSection( &dde_crst );
}
...
/* windows/message.c */
extern void queue_hardware_message( MSG *msg, ULONG_PTR extra_info, enum message_type type );
...
BOOL MSG_peek_message( MSG *msg, HWND hwnd, UINT first, UINT last, int flags )
{
    ...
    for (;;)
    {
        ...
        case MSG_HARDWARE_COOKED:
            ...
            if ((info.type == MSG_HARDWARE_RAW) && !(flags & GET_MSG_REMOVE))
                queue_hardware_message( &info.msg, extra_info, MSG_HARDWARE_COOKED );
            /* fall through */
        case MSG_POSTED:
...
BOOL WINAPI PostMessageW( HWND hwnd, UINT msg, WPARAM wparam, LPARAM lparam )
{
    ...
    if (!hwnd) dest_tid = GetCurrentThreadId();
    else if (!(dest_tid = GetWindowThreadProcessId( hwnd, NULL ))) return FALSE;
