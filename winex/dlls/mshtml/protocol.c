        ...
        DWORD grfPI, DWORD dwReserved)
{
    ...
    BINDINFO bindinfo;
    LPWSTR url_dll, url_file, url, mime, res_type = (LPWSTR)RT_HTMLW;
    HMODULE hdll;
    HRSRC src;
    HRESULT hres;
    ...
    *url_file++ = 0;
    ...
    if(!src) {
        ...
        DWORD_PTR file_id = strtolW(url_file, &endpoint, 10);
        if(endpoint == url_file+strlenW(url_file))
            src = FindResourceW(hdll, (LPCWSTR)file_id, (LPCWSTR)RT_HTMLW);
