        ...
        const char *version, const char *version_string)
{
    ...
    static const WCHAR wszDefaultPath[] = 
        {'M',':','\\','g','e','c','k','o',0};

    if (!mshtml_key)
        strcpyW(gre_path,wszDefaultPath);
    else
    {
        if(version) {
...
/* Set the user-agent - likely across the board for this process.
 * Not ideal, but this is the most obvious access point for the
 * Mozilla code.
 */
void MSHTML_SetGlobalUserAgentA(const char *useragent)
{
    nsIPrefBranch *pref;
    nsresult nsres;
    ...
    nsres = nsIPrefBranch_SetCharPref(pref, "general.useragent.override", useragent);
    if(NS_FAILED(nsres))
        ERR("SetCharPref for user agent failed: %08x\n", nsres);
