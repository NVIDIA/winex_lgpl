...
static HRESULT WINAPI OleControl_OnAmbientPropertyChange(IOleControl *iface, DISPID dispID)
{
    ...
    case DISPID_AMBIENT_USERAGENT:
        ...
        if(FAILED(hres))
            return S_OK;
        /* Actually try to set the user agent.  Currently we
         * can only do this in the scope of our process rather
         * than the scope of this individual control.  Currently
         * this is preferrable to not setting it at all.
         *
         * To guard against this, we save the agent once set and
         * squawk if we try to set it to a different agent later.
         * This will leak if we unload/load this DLL multiple times
         * in a process.
         */
        if(V_VT(&res) == VT_BSTR) {
            static char *last_agent_a = NULL;
            char *agent_a = heap_strdupWtoA(V_BSTR(&res));
            if (!agent_a)
                ERR("failed to convert user_agent to ascii: %s\n",
                    debugstr_w(V_BSTR(&res)));
            else
            {
                if (last_agent_a && strcmp(last_agent_a,agent_a))
                    FIXME("multiple user agents in same process not supported\n");
                MSHTML_SetGlobalUserAgentA(agent_a);
                TRACE("USERAGENT is: %s\n",debugstr_a(agent_a));
                if (!last_agent_a)
                    last_agent_a = agent_a;
                else
                    heap_free(agent_a);
            }
        } else {
            FIXME("unsupported USERAGENT type: %d (%08x)\n", V_VT(&res), V_VT(&res));
        }
        hres = E_FAIL;
        return S_OK;
