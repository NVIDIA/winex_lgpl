...
{
    FIXME("(%p, %s, %s, %s, %s, %p, 0x%08x, %p): stub\n",
...
{
    FIXME("(%08x %p): stub! assuming yes..\n", Reserved, pReserved);
    /* YES! I'm an admin.. would I lie? */
    return TRUE;
}
                                     ...
                                     LPDWORD pdwLSVer, BOOL bVersion )
{
    ...
    {
        *pdwMSVer = LOCALE_NEUTRAL; /* GetUserDefaultUILanguage(); */
        *pdwLSVer = GetACP();
    }
...
void WINAPI RegisterOCX( HWND hWnd, HINSTANCE hInst, LPCSTR cmdline, INT show )
{
    ...
    else
    {
        ...
        if (hr != S_OK)
        {
            ERR("DllRegisterServer entry point returned %08x\n", hr);
        }
    }
