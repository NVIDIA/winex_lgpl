...
static CONST_VTBL IDxDiagProviderVtbl DxDiagProvider_Vtbl =
{
...
static HRESULT fill_processor_information(IDxDiagContainerImpl_Container *node)
{
    ...
    static const WCHAR processor_fmtW[] = {'%','s',' ','@',' ','%','.','2','f','G','H','z',' ','(','%','u',' ','C','P','U','s',')', 0};
    ...
    if(FAILED(hr))
    {
        FIXME("failed to create a WBEM locator object.  WBEM needs to be enabled in the config file in order for DXDiag to work properly! {hr = 0x%08x}\n", hr);
        return hr;
    }
    ...
    sprintfW(print_buf, processor_fmtW, V_BSTR(&cpu_name), V_I4(&clock_speed) / 1000.0f, V_I4(&cpu_no));
...
static HRESULT build_systeminfo_tree(IDxDiagContainerImpl_Container *node)
{
    ...
    static const WCHAR bIsD3DDebugRuntime[] = {'b','I','s','D','3','D','D','e','b','u','g','R','u','n','t','i','m','e',0};
    ...
    FormatMessageW(FORMAT_MESSAGE_FROM_STRING|FORMAT_MESSAGE_ARGUMENT_ARRAY,
                   ...
                   sizeof(print_buf)/sizeof(*print_buf), (va_list*)args);
    ...
    hr = add_bool_property(node, bIsD3DDebugRuntime, FALSE);
    if (FAILED(hr))
        return hr;

    hr = fill_processor_information(node);
    if (FAILED(hr))
        ERR("failed to retrieve the processor information {hr = 0x%08x}\n", hr);
...
{
    typedef IDirect3D9 * (WINAPI *Direct3DCreate9_Func)(UINT);
    ...
    UINT index, count;
    HMODULE module;
    Direct3DCreate9_Func _Direct3DCreate9;


    module = LoadLibraryA("d3d9");

    if (module == NULL)
        return E_FAIL;

    _Direct3DCreate9 = (Direct3DCreate9_Func)GetProcAddress(module, "Direct3DCreate9");

    if (_Direct3DCreate9 == NULL)
        return E_FAIL;

    pDirect3D9 = _Direct3DCreate9(D3D_SDK_VERSION);
    if (!pDirect3D9)
        return E_FAIL;
