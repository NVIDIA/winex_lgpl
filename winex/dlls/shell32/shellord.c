...
typedef BOOL (WINAPI *GetOpenFileNameA_t)(LPOPENFILENAMEA ofn);
...
void WINAPI SHFree(LPVOID x)
{
	...
	if (!SHAllocator) SHGetMalloc(&ppM);
	if (SHAllocator)
	{
	    IMalloc_Free(SHAllocator, x);
	}
	else
	{
...
LPVOID WINAPI SHAlloc(DWORD len)
{
	...
	if (SHAllocator)
	{
	    ret = IMalloc_Alloc(SHAllocator, size);
	}
	else
	{
	    FIXME("couldn't use OLE memory allocator, need to fake our own.\n");
