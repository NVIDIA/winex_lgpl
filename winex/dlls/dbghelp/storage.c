...
void pool_destroy(struct pool* pool)
{
    ...
    struct pool_arena*  next;
    struct pool_arena*  pools[2] = {pool->first, pool->full};
    DWORD               i;
    ...
    {
        for (arena = pools[i]; arena != NULL; arena = arena->next)
        {
            alloc += arena->size;
            used += arena->current - (char*)(arena + 1);
            num++;
        }
    }
    ...
    for (i = 0; i < sizeof(pools) / sizeof(pools[0]); i++)
    {
        for (arena = pools[i]; arena != NULL; arena = next)
        {
            next = arena->next;
...
{
    struct pool_arena * arena;
    struct pool_arena * prev;
    void*               ret;

    len = (len + 3) & ~3; /* round up size on DWORD boundary */

    /* a request for a buffer larger than a single arena was made => satisfy it with a newly
         allocated arena that is the exact requested size. */
    if (len >= pool->arena_size)
    {
        arena = HeapAlloc(GetProcessHeap(), 0, len + sizeof(struct pool_arena));

        if (arena == NULL)
        {
            ERR("failed to allocate %u bytes\n", len);
            return NULL;
        }

        /* add this new arena to the full list immediately. */
        arena->current = (char *)(arena + 1) + len;
        arena->next = pool->full;
        arena->size = len;
        pool->full = arena;
        return arena + 1;
    }

    /* search through the arena list for one that has some space. */
    for (prev = NULL, arena = pool->first; arena; prev = arena, arena = arena->next)
    {
        char *end = (char*)(arena + 1) + arena->size;

        if (end - arena->current >= len)
        {
            ret = arena->current;
            arena->current += len;

            /* we'll consider this arena full and move it to the full list now. */
            if (end - arena->current <= POOL_FULL_THRESHOLD)
            {
                /* move the arena to the full list so we don't need to be searching it any more. */
                if (prev != NULL)
                    prev->next = arena->next;

                else
                    pool->first = arena->next;
    ...
    arena = HeapAlloc(GetProcessHeap(), 0, pool->arena_size + sizeof(struct pool_arena));
    if (!arena) {FIXME("OOM\n");return NULL;}
