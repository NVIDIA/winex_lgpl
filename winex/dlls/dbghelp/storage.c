...
void pool_destroy(struct pool* pool)
{
    ...
    while (arena)
    {
        ...
        used += arena->current - (char*)arena - sizeof(struct pool_arena);
        num++;
        arena = arena->next;
    }
...
void* pool_alloc(struct pool* pool, unsigned len)
{
    ...
    assert(len <= pool->arena_size && len);

    for (arena = pool->first; arena; arena = arena->next)
    {
        if ((char*)arena + sizeof(struct pool_arena) + pool->arena_size - arena->current >= len)
        {
            ret = arena->current;
            arena->current += len;
            return ret;
