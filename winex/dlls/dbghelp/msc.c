...
static void codeview_init_basic_types(struct module* module)
{
    ...
    cv_basic_types[T_RESERVED].basic  = &symt_new_basic(module, btVoid,  "reserved", 4)->symt;
    ...
    cv_basic_types[T_RESERVED].near32Ptr= &symt_new_pointer(module, cv_basic_types[T_RESERVED].basic)->symt;
...
static BOOL check_for_large_pdb(HANDLE h)
{
    typedef int (WINAPI *MessageBoxA_Func)(HWND, LPCSTR, LPCSTR, UINT);
    LARGE_INTEGER       pdbSize;
    int                 answer;
    MessageBoxA_Func    pMessageBoxA = NULL;
    HMODULE             user32;


    /* config says to always load large PDB files => succeed.  This allows a workaround for
         some situations where showing a message box might not be allowed or possible but
         we still want to be able to get info out of dbghelp.  One example of this case would
         be early on process startup while libraries are still being initialized. */
    if (g_alwaysLoadLargePDBs)
        return TRUE;

    pdbSize.QuadPart = 0;
    GetFileSizeEx(h, &pdbSize);

    /* this PDB is rather large => ask the user whether they want to load it. */
    if (pdbSize.QuadPart <= 20 * 1024 * 1024)
        return TRUE;
    ...
    if (user32 != NULL)
        pMessageBoxA = (MessageBoxA_Func)GetProcAddress(user32, "MessageBoxA");

    /* failed to grab the function -> the process may not have loaded user32 => allow the PDB
         to load as normal. */
    if (pMessageBoxA == NULL)
        return TRUE;

    answer = pMessageBoxA(NULL,
                         "This debug information file is rather large and could take several minutes to load.  Would you like to continue loading it?",
                         "Large Debug Information File",
                         MB_YESNO);
                            ...
                            struct module* module)
{
    ...
    TRACE(".PDB '%s': '%s' returns %p\n", lookup->filename, dbg_file_path, h);

    if (h != INVALID_HANDLE_VALUE)
    {
        /* ask the user if it is okay to load a large PDB file. */
        if (!check_for_large_pdb(h))
        {
            CloseHandle(h);
            h = NULL;
        }
