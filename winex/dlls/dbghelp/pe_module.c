...
static BOOL pe_load_stabs_helper(struct module *module, ULONG_PTR offset, const BYTE *_stab, ULONG stabSize, const char *stabStr, ULONG stabStrSize)
{
    ...
    TRACE("{module = %p, offset = %tu, _stab = %p, stabSize = %u, stabStr = %p, stabStrSize = %u, sizeof(StabEntry) = %zu}\n",
    ...
    while (stab < stabEnd && str < strEnd)
    {
        ...
        TRACE("checking the first STAB entry at offset 0x%08tx {entry = %p {n_strx = %d, n_type = 0x%02x, n_desc = 0x%04x, n_value = %u}, size = %u, str = %p, strSize = %u}\n",
        ...
        /* parse the subtables. */
        TRACE("parsing the STABS table at offset 0x%08tx {stab = %p, size = %u, str = %p, strSize = %u}\n", stab - _stab, stab, size, str, strSize);
        success = stabs_parse(module, offset, stab, size, str, strSize);

        if (!success)
        {
            ERR("failed to parse the STABS subtable at offset 0x%08tx {stab = %p, size = %u, str = %p, strSize = %u}\n", stab - _stab, stab, size, str, strSize);
            result = FALSE;
        }
