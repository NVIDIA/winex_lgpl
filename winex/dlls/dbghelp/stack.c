...
static BOOL stack_walk(struct stack_walk_callback* cb, LPSTACKFRAME frame)
{
    ...
    else
    {
        ...
        {
            WARN("[next32] Cannot read new frame offset %08x\n", frame->AddrFrame.Offset + (int)sizeof(DWORD));
            goto done_err;
        }
                        ...
                        PTRANSLATE_ADDRESS_ROUTINE64 f_xlat_adr)
{
    ...
    TRACE("(MachineType = %u, hProcess = 0x%08x, hThread = 0x%08x, frame64 = %p, ctx = %p, f_read_mem = %p, FunctionTableAccessRoutine = %p, GetModuleBaseRoutine = %p, f_xlat_adr = %p)\n",
