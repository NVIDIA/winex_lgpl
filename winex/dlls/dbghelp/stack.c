...
static inline BOOL sw_read_mem(struct stack_walk_callback* cb, DWORD addr, void* ptr, DWORD sz)
{
    ...
    if (cb->is32)
        return cb->u.s32.f_read_mem(cb->hProcess, addr, ptr, sz, &bytesRead);
    else
        return cb->u.s64.f_read_mem(cb->hProcess, addr, ptr, sz, &bytesRead);
}
...
static BOOL stack_walk(struct stack_walk_callback* cb, LPSTACKFRAME frame, DWORD_PTR espValue)
{
    ...
    if (curr_mode == stm_start)
    {
        ...
        {
            TRACE("detected a failed call instruction.  Marking the first stack frame as incomplete {AddrPC = %s, AddrFrame = %s, espValue = 0x%08tx}\n",
    ...
    else
    {
        ...
        {
            WARN("[next32] Cannot read new frame offset %08tx\n", returnAddress);
            goto done_err;
        }
