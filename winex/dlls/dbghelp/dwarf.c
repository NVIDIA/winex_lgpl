    ...
    while (ctx->data < ctx->end_data)
    {
        ...
        case DW_OP_deref:
            ...
            if (hproc)
            {
                ...
                {
                    WARN("Couldn't read memory at %tx\n", addr);
        ...
        {
            /* the value on top of the stack is to be treated as an offset into the compile unit's TLS slot.  The value in the
               referenced offset of the TLS slot will replace the top of the stack.  This extension seems to have become a part
               of the DWARF spec in version 3 as the operation DW_OP_form_tls_address. */
            /* FIXME!! the DWARF spec leaves the computation of the TLS slot address up to the caller.  There is no standard
                       way to access it.  Note that this is the compiler generated TLS slot for the module, not the windows
                       TLS slot for the module.  The spec mentions that some compilers will emit a function that can be used
                       to access the module's TLS slot, but finding/calling that is more difficult than need be.

                       The basic operation here is to do the following:
                        - find the current module's TLS slot and retrieve its value (<tlsPtr>).  The compiler may have emitted
                          a function to handle this for us, but we need to find the pointer to that function.
                        - pop the value off the top of the stack (<offset>).
                        - the value just popped is to be treated as an offset into the TLS block.
                        - push the value "<tlsPtr> + <offset>" back onto the stack.

                       The next operation is responsible for deciding what to do with the value (ie: take it as a literal,
                       dereference it, etc).

                       For now, we'll just leave the offset value in place on the top of the stack and continue with processing
                       the operation sequence.  This may lead to more crashiness 
            */
            DWORD_PTR offset = stack[stk--];
            DWORD_PTR tlsPtr;


            FIXME("find out how to access the module's TLS data {op = DW_OP_GNU_push_tls_address, offset = 0x%08tx}\n", offset);
            tlsPtr = 0;
            stack[++stk] = tlsPtr + offset;
            break;
        }
        default:
                                            ...
                                            struct symt_compiland* compiland)
{
    ...
    if (!is_decl.u.uvalue)
    {
        subpgm.func = symt_new_function(ctx->module, compiland, name.u.string ? name.u.string : "(null)",
                                          ...
                                          unsigned long load_offset)
{
    ...
    TRACE("Compilation Unit Header found at 0x%tx:\n",
