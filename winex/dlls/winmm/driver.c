...
LPWINE_DRIVER	DRIVER_TryOpenDriver32(LPCSTR fn, LPARAM lParam2)
{
 ...
 exit:
    ...
    WARN("Unable to load 32 bit module %s: %s\n", debugstr_a(fn), cause);
    return NULL;
}
...
HDRVR WINAPI OpenDriverA(LPCSTR lpDriverName, LPCSTR lpSectionName, LPARAM lParam2)
{
    ...
    /* Try the aliases first */
    if (lsn == NULL) {
        lstrcpynA(libName2, lpDriverName, sizeof(libName2));
        lsn = "Drivers32";
    }
    ...
    if ((lpDrv = DRIVER_TryOpenDriver32(libName2, lParam2)))
        goto the_end;
