...
static	BOOL	MULTIMEDIA_CreateIData(HINSTANCE hInstDLL)
{
    ...
    CRITICAL_SECTION_NAME( &iData->cs, (void*)__FILE__ ": WinMM" );
...
static	void MULTIMEDIA_DeleteIData(void)
{
    ...
    TRACE("Deleting IData (%p) for pid %08lx\n", iData, iData->dwThisProcess);
    if (iData) {
	TIME_MMTimeStop();
...
BOOL WINAPI WINMM_LibMain(HINSTANCE hInstDLL, DWORD fdwReason, LPVOID fImpLoad)
{
	...
	int threads = 0;
	WINE_PLAYSOUND* lpPlaySound = iData->lpPlaySound;
	/* wait for threads to finish.
	 * FIXME - should we send the stop event to them?
	 *       - if you are getting very long application quitting times,
	 *         it could be this - so do that.
	 *
	 *    Alternatively, make the last thread delete this stuff.. but
	 *    that just gets ugly.
	 */
	EnterCriticalSection(&(iData->cs));
	while (lpPlaySound)
	{
	    if (lpPlaySound->hThread) threads++;
	    lpPlaySound = lpPlaySound->lpNext;
	}
	if (threads)
	{
	    int i;
	    HANDLE *hThreads = HeapAlloc(GetProcessHeap(),
					 HEAP_ZERO_MEMORY,
					 sizeof(HANDLE) * threads);
	    lpPlaySound = iData->lpPlaySound;
	    threads = 0;
	    while (lpPlaySound)
	    {
		if (lpPlaySound->hThread)
	    ...
	    /* its safe to leave the lock, as apart apart from the threads that
	     * we are about to wait on, none can change (ie, none added)
	     * because this is in process detach */
...
DWORD WINAPI proc_PlaySound(LPVOID arg)
{
    ...
    char*		sndData = NULL;
    INT			count, bufsize, left, index, sndSize;
    ...
    /* make it so that 3 play buffers per second are needed */
    bufsize = (((lpWaveFormat->nAvgBytesPerSec / 3) - 1) / lpWaveFormat->nBlockAlign + 1) *
	lpWaveFormat->nBlockAlign;
    waveHdr = HeapAlloc(GetProcessHeap(), 0, 2 * sizeof(WAVEHDR));
    waveHdr[0].lpData = waveHdr[1].lpData = NULL;
    ...
    sndSize = mmckInfo.cksize;
    if (wps->fdwSound & SND_MEMORY) {
        /* sound is already in memory... avoid the copy */
     	sndData = data + mmckInfo.dwDataOffset; 
    } else {
	/* alloc a buffer and read in the entire sound */
    	sndData = HeapAlloc(GetProcessHeap(), 0, sndSize);
	count = mmioRead(hmmio, sndData, sndSize);
	if( count != sndSize ) {
	  WARN("error reading sound\n");
	  goto errCleanUp;
	}
    }
	...
	while (left)
	    ...
	    count = min(bufsize, left);
	    if (count < 1) break;

	    if( waveHdr[index].lpData != NULL ) {
	    	waveOutUnprepareHeader(hWave, &waveHdr[index], sizeof(WAVEHDR));
	    }
	    waveHdr[index].lpData = sndData + (sndSize-left);
	    waveHdr[index].dwBufferLength = count;
	    waveHdr[index].dwFlags &= ~WHDR_DONE;
	    if( waveOutPrepareHeader(hWave, &waveHdr[index], sizeof(WAVEHDR)) ) {
	        goto errCleanUp;
	    }
...
errCleanUp:
    ...
    if (sndData && !(wps->fdwSound & SND_MEMORY)) HeapFree(GetProcessHeap(), 0, sndData);
 ...
 */
MMRESULT WINAPI MMSYSTEM_MidiStream_Open(HMIDISTRM* lphMidiStrm, LPUINT lpuDeviceID,
...
UINT WINAPI MMSYSTEM_waveOpen(HANDLE* lphndl, UINT uDeviceID, UINT uType,
				      ...
				      DWORD dwFlags, BOOL bFrom32)
{
    ...
    {
        /* opening normally, so allocate  driver handle */
    ...
    {
        /* reopening to change format, so just use the handle provided */
        handle = *lphndl;
        if ((wmld = MMDRV_Get(handle, MMDRV_WAVEOUT, FALSE)) == NULL)
            return MMSYSERR_INVALHANDLE;
    }
 ...
 *                     wine_mmio_swap32
 *
 * Workaround a compiler bug that affects mmio.c by forcing compiler to
 * out-of-line the swapinplace calls
 * FIXME - is this still a problem?
 */
void wine_mmio_swap32(void *base)
{
    _swapinplace32(base);
}
