...
typedef DWORD (WINAPI *VerQueryValueA_t)(LPVOID,LPCSTR,LPVOID*,UINT*);
...
static	BOOL	MMDRV_GetDescription32(const char* fname, char* buf, int buflen)
{
    ...
    VerQueryValueA_t pVerQueryValueA;
    HMODULE hmodule = 0;
...
{
    LPWINE_MLD	mld;
    LPDWORD pDwordHandle = (LPDWORD)hndl;
	...
	if (!MM_MLDrvs[*pDwordHandle]) break;
    }
    ...
    MM_MLDrvs[*pDwordHandle] = mld;
    *pDwordHandle = *pDwordHandle | 0x8000;
...
void	MMDRV_Free(HANDLE hndl, LPWINE_MLD mld)
{
	...
	unsigned idx = (UINT_PTR)hndl & ~0x8000;
...
LPWINE_MLD	MMDRV_Get(HANDLE hndl, UINT type, BOOL bCanBeID)
{
	...
	if ((UINT_PTR)hndl & 0x8000) {
	    hndl = (UINT_PTR)hndl & ~0x8000;
	    if (hndl < sizeof(MM_MLDrvs) / sizeof(MM_MLDrvs[0])) {
		mld = MM_MLDrvs[(UINT_PTR)hndl];
...
#ifdef HAVE_MACOSX_COREAUDIO
    MMDRV_Install("winemacca.drv",     "winemacca.drv",        FALSE);
#else
