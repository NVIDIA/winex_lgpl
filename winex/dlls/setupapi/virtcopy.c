					...
					LPARAM lParam, LPARAM lParamRef)
{
    static int count = 0;
    if (count < 10)
        FIXME("(%p, %04x, %04tx, %08tx, %08tx) - what to do here ?\n",
...
static BOOL VCP_UI_GetDialogTemplate(LPCVOID *template32)
{
    ...
    if (!(hResInfo = FindResourceA(SETUPAPI_hInstance, MAKEINTRESOURCEA(COPYFILEDLGORD), (LPSTR)RT_DIALOGA)))
	return FALSE;
					...
					LPARAM lParam, LPARAM lParamRef)
{
    ...
    if (count < 5)
        FIXME("(%p, %04x, %04tx, %08tx, %08tx) - semi-stub\n",
		lpvObj, uMsg, wParam, lParam, lParamRef);
    count++;
    switch (uMsg)
    {
