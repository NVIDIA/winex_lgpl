...
static struct inf_file *parse_file( HANDLE handle, const WCHAR *class, UINT *error_line )
{
    ...
    if (!IsTextUnicode( buffer, size, NULL ))
    {
                                  ...
                                  DWORD size, PDWORD required )
{
    ...
    if (buffer)
    {
        ...
        TRACE( "context %p/%p/%d/%d index %u returning %s\n",
                                 ...
                                 DWORD size, LPDWORD required )
{
    ...
    {
        DPRINTF( "%p/%p/%d/%d index %u returning",
                                     ...
                                     DWORD ReturnBufferSize, PDWORD RequiredSize)
{
	...
	break;
      ...
      default:
	FIXME("SearchControl=%u not implemented yet!\n", SearchControl);
	break;
    }

    if( !file ) {
      ERR("could not locate requested INF file\n");
    ...
    req_size = inf_count * sizeof(SP_INF_INFORMATION);
    if( ReturnBuffer == NULL /* && (ReturnBufferSize == 0) */)
    {
      if( RequiredSize ) 
	*RequiredSize = req_size;
      TRACE("setting RequiredSize=%u\n", req_size);
      return TRUE;
    }
    
    if( req_size < ReturnBufferSize ) {
      if( RequiredSize )
	*RequiredSize = req_size;
      TRACE("insufficient buffer space, given=%u, req=%u\n", ReturnBufferSize, req_size);
      SetLastError(ERROR_INSUFFICIENT_BUFFER);
      return FALSE;
    }
    ...
    inf_count = 0;
    while( cur ) {
      /* should actually parse the given INF file to determine this info.. */
      ReturnBuffer[inf_count].InfStyle = INF_STYLE_NONE;
      ReturnBuffer[inf_count].InfCount = 1;
      memset(ReturnBuffer[inf_count].VersionData, 0, sizeof(ReturnBuffer[inf_count].VersionData));
