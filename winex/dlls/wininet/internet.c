...
static const WCHAR url_schemes[][11] =
{
    {'f','t','p',0},
    {'g','o','p','h','e','r',0},
    {'h','t','t','p',0},
...
static BOOL SetUrlComponentValueW(LPWSTR* lppszComponent, LPDWORD dwComponentLen, LPCWSTR lpszStart, DWORD len)
{
    ...
    if (*dwComponentLen != 0 || *lppszComponent == NULL)
    {
        ...
        {
            DWORD ncpylen = min((*dwComponentLen), len);
            memcpy(*lppszComponent, lpszStart, ncpylen*sizeof(WCHAR));

            /* native does not guarantee termination of the buffer if it is too small */
            if (ncpylen < *dwComponentLen)
            {
                (*lppszComponent)[ncpylen] = '\0';
                *dwComponentLen = ncpylen;
            }
                              ...
                              LPURL_COMPONENTSW lpUC)
{
   ...
   */
    ...
    if (bIsAbsolute) /* Parse <protocol>:[//<net_loc>] */
    {
        ...
        /* native sets the ERROR_INVALID_PARAMETER error code if the scheme buffer
           length is negative and the scheme buffer pointer is NULL.  The call still
           succeeds in this case. */
        if ((INT)lpUC->dwSchemeLength < 0 && lpUC->lpszScheme == NULL)
            SetLastError(ERROR_INVALID_PARAMETER);

        if (!SetUrlComponentValueW(&lpUC->lpszScheme, &lpUC->dwSchemeLength, lpszUrl, lpszcp - lpszUrl))
        {
...
DWORD WINAPI PrivacyGetZonePreferenceW(DWORD dwZone, DWORD dwType,
            LPDWORD pdwTemplate, LPWSTR pszBuffer, LPDWORD pdwBufferLength)

{
    FIXME("Stub: %d %d %p %p %p\n", dwZone, dwType, pdwTemplate, pszBuffer,
          pdwBufferLength);
...
DWORD WINAPI PrivacySetZonePreferenceW(DWORD dwZone, DWORD dwType,
            DWORD dwTemplate, LPCWSTR pszPreference)
{
