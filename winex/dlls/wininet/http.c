    ...
    do
    {
        ...
        if (*lpszEnd == '\r' || *lpszEnd == '\n')
        {
            *lpszEnd = '\0';
            ...
            /* jump over all consecutive newlines and carriage returns. */
            while (*lpszEnd == '\n' || *lpszEnd == '\r')
                lpszEnd++;
        }
...
static void HTTPREQ_Destroy(object_header_t *hdr)
{
    ...
    heap_free(request->hostName);
    heap_free(request->userName);
    heap_free(request->password);
...
static DWORD HTTPREQ_QueryOption(object_header_t *hdr, DWORD option, void *buffer, DWORD *size, BOOL unicode)
{
    ...
    case INTERNET_OPTION_URL: {
        ...
        static const WCHAR httpW[] = {'h', 't', 't', 'p', ':', '/', '/', '%', 's', '%', 's', 0};
        static const WCHAR httpsW[] = {'h', 't', 't', 'p', 's', ':', '/', '/', '%', 's', '%', 's', 0};

        TRACE("INTERNET_OPTION_URL\n");

        host = HTTP_GetHeader(req, hostW);
        snprintfW(url, INTERNET_MAX_URL_LENGTH, req->hostPort == INTERNET_DEFAULT_HTTPS_PORT ? httpsW : httpW, host->lpszValue, req->path);
    ...
    case INTERNET_OPTION_USERNAME:
        return str_to_buffer(req->userName, buffer, size, unicode);
    case INTERNET_OPTION_PASSWORD:
        return str_to_buffer(req->password, buffer, size, unicode);
...
static DWORD HTTPREQ_SetOption(object_header_t *hdr, DWORD option, void *buffer, DWORD size)
{
    ...
    case INTERNET_OPTION_USERNAME:
        heap_free(req->userName);
        if (!(req->userName = heap_strdupW(buffer))) return ERROR_OUTOFMEMORY;
        return ERROR_SUCCESS;

    case INTERNET_OPTION_PASSWORD:
        heap_free(req->password);
        if (!(req->password = heap_strdupW(buffer))) return ERROR_OUTOFMEMORY;
        return ERROR_SUCCESS;
        ...
        DWORD dwFlags, DWORD_PTR dwContext, HINTERNET *ret)
{
    ...
    /* copy the port, host, username, and password information from the session.  This is done so
       that the session object is never clobbered and can therefore be used as a 'template' for
       creating new requests even after a previous request was redirected.  This is needed for
       some apps that only create a single session object and connection but make several
       requests on it. */
    request->hostPort = session->hostPort;
    request->hostName = heap_strdupW(session->hostName);
    request->userName = heap_strdupW(session->userName);
    request->password = heap_strdupW(session->password);
    list_add_head( &session->hdr.children, &request->hdr.entry );

    request->server = get_server(request->hostName, request->hostPort, (dwFlags & INTERNET_FLAG_SECURE) != 0, TRUE);
    if(!request->server) {
        WININET_Release(&request->hdr);
        return ERROR_OUTOFMEMORY;
    }
    ...
    if (hIC->proxy && hIC->proxy[0] && !HTTP_ShouldBypassProxy(hIC, request->hostName))
    ...
    case HTTP_QUERY_RAW_HEADERS_CRLF:
        {
            ...
            /* add an extra two characters of space for a terminating blank line (ie: CRLF).  Many
               apps expect this blank line as a terminator for their header parsing loops. */
            len = (strlenW(headers) + 2) * sizeof(WCHAR);
            ...
            else if (lpBuffer)
            {
                ...
                /* the headers must always be terminated with a blank line.  Add that now. */
                memcpy(&((BYTE *)lpBuffer)[len], szCrLf, sizeof(szCrLf));
...
#undef FE
	...
	TRACE("(%p, 0x%08x, %p, %p (%d), %p (%d))--> %d\n",
        hHttpRequest, dwInfoLevel, lpBuffer, lpdwBufferLength, lpdwBufferLength ? *lpdwBufferLength : -1,
        lpdwIndex, lpdwIndex ? *lpdwIndex : -1, info);
	TRACE("  Attribute:");
...
static LPWSTR HTTP_GetRedirectURL(http_request_t *request, LPCWSTR lpszUrl)
{
    ...
    urlComponents.lpszUserName = request->userName;
...
static DWORD HTTP_HandleRedirect(http_request_t *request, LPCWSTR lpszUrl)
{
    ...
    else
    {
        ...
        if(custom_port) {
            ...
            request->hostName = heap_alloc(len*sizeof(WCHAR));
            sprintfW(request->hostName, fmt, hostName, urlComponents.nPort);
        }
        else
            request->hostName = heap_strdupW(hostName);
        request->hostPort = urlComponents.nPort;

        HTTP_ProcessHeader(request, hostW, request->hostName, HTTP_ADDREQ_FLAG_ADD | HTTP_ADDREQ_FLAG_REPLACE | HTTP_ADDHDR_FLAG_REQ);

        heap_free(request->userName);
        request->userName = NULL;
        if (userName[0])
            request->userName = heap_strdupW(userName);
...
static void HTTP_InsertCookies(http_request_t *request)
{
    ...
    if(get_cookie(host->lpszValue, request->path, NULL, NULL, &cookie_size, INTERNET_COOKIE_HTTPONLY) != ERROR_SUCCESS)
        return;
    ...
    get_cookie(host->lpszValue, request->path, NULL, cookies+cnt, &cookie_size, INTERNET_COOKIE_HTTPONLY);
	...
	DWORD dwContentLength, BOOL bEndRequest)
{
    ...
    if ((dwContentLength && !bEndRequest) || strcmpW(request->verb, szGET))
    {
