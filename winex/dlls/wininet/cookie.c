...
{
    BOOL ret;
    struct list * domainCursor;
    struct list * domainCursor2;
    ...
    LIST_FOR_EACH_SAFE(domainCursor, domainCursor2, &domain_list)
    {
        cookie_domain *cookiesDomain = LIST_ENTRY(domainCursor, cookie_domain, entry);
    ...
    LPSTR lpCookieData, LPDWORD lpdwSize)
{
    ...
    url = HEAP_strdupAtoW(GetProcessHeap(), 0, lpszUrl);
    name = HEAP_strdupAtoW(GetProcessHeap(), 0, lpszCookieName);
