...
#else
    /* FIXME!! implement IdnToUnicode(). */
    part_len = uc.dwHostNameLength;

    if (decoded_url != NULL)
        memcpy(decoded_url + len, host_name, (part_len + 1) * sizeof(WCHAR));
#endif
    heap_free(host_name);
    if(!part_len) {
...
#else
    /* FIXME!! implement IdnToAscii(). */
    memcpy(punycode, uc.lpszHostName, (part_len + 1) * sizeof(WCHAR));
#endif
    if(!part_len) {
        heap_free(punycode);
        return 0;
