...
void computeInternalFrame(CodecInfo *pi, LPCBITMAPINFOHEADER lpbiIn, LPBYTE lpIn)
{
  ...
  {
    const RGBQUAD *lp = (const RGBQUAD*)((const BYTE*)lpbiIn + lpbiIn->biSize);
				 ...
				 LPBITMAPINFOHEADER lpbiOut)
{
  ...
  if (lpbiOut != NULL) {
    ...
    memcpy((LPBYTE)lpbiOut + lpbiOut->biSize, (const BYTE*)lpbiIn
	   + lpbiIn->biSize, lpbiOut->biClrUsed * sizeof(RGBQUAD));
...
static LRESULT CompressEnd(CodecInfo *pi)
{
  ...
  if (pi != NULL) {
    if (pi->pPrevFrame != NULL)
      (void)GlobalFreePtr(pi->pPrevFrame);
    if (pi->pCurFrame != NULL)
      (void)GlobalFreePtr(pi->pCurFrame);
    pi->pPrevFrame = NULL;
    pi->pCurFrame  = NULL;
    pi->nPrevFrame = -1;
    pi->bCompress  = FALSE;
				    ...
				    LPBITMAPINFOHEADER lpbiOut)
{
  ...
  if (lpbiIn->biBitCount <= 8) {
    ...
    memcpy((LPBYTE)lpbiOut + lpbiOut->biSize, (const BYTE*)lpbiIn
	   + lpbiIn->biSize, size * sizeof(RGBQUAD));
