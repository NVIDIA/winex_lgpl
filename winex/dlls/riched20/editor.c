...
BOOL WINAPI DllMain(HINSTANCE hinstDLL, DWORD fdwReason, LPVOID lpvReserved)
{
    ...
    case DLL_PROCESS_ATTACH:
      ...
      me_heap = HeapCreateInternal (0, 0x10000, 0, "riched20 Heap");
...
LRESULT WINAPI RichEditANSIWndProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam) {
  ...
  TRACE("hWnd %p msg %04x (%s) %08tx %08tx\n",
...
void ME_SendOldNotify(ME_TextEditor *editor, int nCode)
{
  ...
  SendMessageA(GetParent(hWnd), WM_COMMAND, (nCode<<16)|GetWindowLongPtrW(hWnd, GWLP_ID), (LPARAM)hWnd);
}
...
void ME_LinkNotify(ME_TextEditor *editor, UINT msg, WPARAM wParam, LPARAM lParam)
{
  ...
  ENLINK info;
  TRACE("editor: %p, msg: %u, wParam: %tx, lParam: %tx\n", editor, msg, wParam, lParam);
...
void ME_RegisterEditorClass(HINSTANCE hInstance)
{
  ...
  wcW.hIcon = NULL;
  wcW.hCursor = LoadCursorW(NULL, MAKEINTRESOURCEW(IDC_IBEAMW));
  ...
  wcA.hIcon = NULL;
  wcA.hCursor = LoadCursorW(NULL, MAKEINTRESOURCEW(IDC_IBEAMW));
