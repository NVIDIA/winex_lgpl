			   ...
			   DWORD dwFlags)
{
    ...
    *preliminary = L'\0';
    ...
    ret = S_OK;
    appendSlash = (!(dwFlags & URL_PLUGGABLE_PROTOCOL) && URL_JustLocation(relative.ap2));
    ...
    case 1:  /*
	      ...
	      */
	len = strlenW(mrelative) + strlenW(preliminary) + 1;
	if (len > *pcchCombined) {
	    *pcchCombined = len;
	    ret = E_POINTER;
	    break;
	...
	len = strlenW(mrelative) + 1 + 
          (appendSlash ?
            ((sizeof(single_slash) / sizeof(single_slash[0])) - 1) : 
            0);
    ...
    case 3:  /*
	      ...
	      * Note that this calculation includes the terminating null.
	      */
	len = base.sizep1 + 1 + 
	      relative.sizep2 + 1 +
	      (appendSlash ? 
    ...
    /* copy the scheme name plus the colon (make sure to leave space for the terminating null!) */
    lstrcpynW(preliminary, base.ap1, base.sizep1 + 1 + 1);
	...
	len = base.sizep1 + 1 + 
	      sizeloc + 
	      relative.sizep2 + 1 + 
	      ((dwFlags & URL_PLUGGABLE_PROTOCOL) ? 1 : 0);
	if (len > *pcchCombined) {
	    *pcchCombined = len;
	    ret = E_POINTER;
	    break;
	}
	lstrcpynW(preliminary, base.ap1, base.sizep1+1+sizeloc + 1);
    ...
    case 5:  /*
	      ...
	      */
	len = base.sizep1 + 1 + base.sizep2 + 
	      relative.sizep2 + 1 + 
	      1;    /* a slash could be appended to the string */
	if (len > *pcchCombined) {
	    *pcchCombined = len;
	    ret = E_POINTER;
	    break;
	}
	lstrcpynW(preliminary, base.ap1, base.sizep1+1+base.sizep2 + 1);
	...
	DWORD dwFlags)
{
    ...
    for(src = pszUrl, needed = 0; *src; src++, needed++) {
	   ...
	   (*src == '#' || *src == '?')) {
	    ...
	    INT ih;
	    char buf[5] = "0x";
	    memcpy(buf+2, src + 1, 2);
	    buf[4] = '\0';
	    StrToIntExA(buf, STIF_SUPPORT_HEX, &ih);
	    next = (CHAR) ih;
