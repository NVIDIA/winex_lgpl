 ...
 void *pvStructInfo, DWORD *pcbStructInfo)
{
    ...
    __TRY
    {
        SPC_SP_OPUS_INFO *info = pvStructInfo;
        struct AsnDecodeSequenceItem items[] = {
         { ASN_CONTEXT | ASN_CONSTRUCTOR,
           offsetof(SPC_SP_OPUS_INFO, pwszProgramName),
           CRYPT_AsnDecodeBMPString, sizeof(LPCWSTR),
           TRUE, TRUE, offsetof(SPC_SP_OPUS_INFO, pwszProgramName), 0},
         { (ASN_CONTEXT | ASN_CONSTRUCTOR | 1),
           offsetof(SPC_SP_OPUS_INFO, pMoreInfo),
           CRYPT_AsnDecodeSPCLinkPointer, sizeof(PSPC_LINK),
           TRUE, TRUE, offsetof(SPC_SP_OPUS_INFO, pMoreInfo), 0},
         { (ASN_CONTEXT | ASN_CONSTRUCTOR | 2),
           offsetof(SPC_SP_OPUS_INFO, pPublisherInfo),
           CRYPT_AsnDecodeSPCLinkPointer, sizeof(PSPC_LINK),
           TRUE, TRUE, offsetof(SPC_SP_OPUS_INFO, pPublisherInfo), 0},
        };
        ...
        /* work around an extra character that sneaks into the program name */
        if (info && info->pwszProgramName)
