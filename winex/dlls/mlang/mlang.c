...
static BOOL CALLBACK enum_locales_proc(LPWSTR locale)
{
    ...
    if (data->total >= data->allocated)
    {
        ...
        if (data->info == NULL)
            tmp = HeapAlloc(GetProcessHeap(), 0, data->allocated * sizeof(RFC1766INFO));

        else
            tmp = HeapReAlloc(GetProcessHeap(), 0, data->info, data->allocated * sizeof(RFC1766INFO));

        if (tmp == NULL)
        {
            ERR("failed to allocate memory for a new locale\n");
            return FALSE;
        }
