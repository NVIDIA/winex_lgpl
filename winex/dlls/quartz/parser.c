...
HRESULT CParserImplThread_ProcessNextSample( CParserImpl* This )
{
	...
	if ( pOutPin != NULL && pOutPin->m_bReqUsed )
	{
		if ( This->m_pHandler->pProcessSample != NULL ) {
			...
			TRACE("ProcessSample returns %08x\n", hr);
		}
		...
		if ( SUCCEEDED(hr) )
		{
				 ...
				 pOutPin->m_pOutPinAllocator != This->m_pAllocator )
			{
				...
				if ( SUCCEEDED(hr) )
				{
					...
					TRACE("Copy returns %08x\n", hr);
					if ( SUCCEEDED(hr) ) {
						hr = CPinBaseImpl_SendSample(&pOutPin->pin,pSample);
						TRACE("SendSample returns %08x\n", hr);
					}
...
DWORD WINAPI CParserImplThread_Entry( LPVOID pv )
{
	...
	while ( 1 )
	{
		if ( bReqNext )
		{
			...
			/* the AsyncReader uses these "time stamps" as file offsets */
		...
		if ( !This->m_ppOutPins[nIndex]->m_bReqUsed )
		{
			...
			if ( SUCCEEDED(hr) )
				hr = IMediaSample_SetTime(pSample,&rtReqStart,&rtReqStop);
			if ( FAILED(hr) )
			{
				CParserImplThread_ErrorAbort(This,hr);
				break;
			...
			This->m_ppOutPins[nIndex]->m_rtReqStart = rtReqStart;
			This->m_ppOutPins[nIndex]->m_rtReqStop = rtReqStop;
...
BOOL CParserImpl_EndThread( CParserImpl* This, BOOL bAsync )
{
	...
	if ( This->m_hEventInit != (HANDLE)NULL )
	{
		...
		/* send a flush notification outside the processing thread to reduce
		   the latency when stopping the filter */
		CParserImplThread_SendFlush(This);
...
static HRESULT CParserImpl_OnInactive( CBaseFilterImpl* pImpl )
{
	...
	if ( FAILED(hr) )
	{
		FIXME("CParserImpl_BeginThread returns %08x\n",hr);
		CParserImpl_EndThread(This,FALSE);
		return hr;
	}
...
static HRESULT CParserImpl_OnStop( CBaseFilterImpl* pImpl )
{
	...
	if ( !CParserImpl_EndThread(This,FALSE) ) /* FIXME: bAsync=TRUE isn't stable yet */
	...
	const ParserHandlers* pHandler )
{
	...
	CRITICAL_SECTION_DEFINE( &This->m_csParser );
