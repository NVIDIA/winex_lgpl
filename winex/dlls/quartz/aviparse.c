...
#define KEYFRAMES
#define AUDIO_TIME (QUARTZ_TIMEUNITS/2)		/* deliver audio frames 0.5s before video frames */
...
static HRESULT CAVIParseImpl_InitParser( CParserImpl* pImpl, ULONG* pcStreams )
{
	...
	{
		TRACE("search strl for stream %u\n",nIndex);
		hr = RIFF_SearchList(
			pImpl,
        ...
        /* Find the movi list, and store the offset */
	hr = RIFF_SearchList(
		pImpl,
		(DWORD)0xffffffff, PARSER_RIFF_OfsFirst, PARSER_movi,
		&This->movieChunkOfs, &dwChunkLength );
	if ( FAILED(hr))
		This->movieChunkOfs = 0;

	if (This->movieChunkOfs)
		This->movieChunkOfs -= 4;  /* Don't include the riff header ? */

	TRACE("Movie offset: %lld\n", This->movieChunkOfs);
...
{
	CAVIParseImpl*	This = (CAVIParseImpl*)pImpl->m_pUserData;
	REFERENCE_TIME	rtNext, rtDeliver, rtCur;
	DWORD	nIndexNext;
	DWORD	nIndex;
	...
	if ( This == NULL )
		return E_UNEXPECTED;
#if defined(NO_AUDIO) || defined(NO_VIDEO)
do_it_again:
#endif
#ifdef KEYFRAMES
	*pdwSampleFlags = 0;
	...
	*pdwSampleFlags = AM_SAMPLE_SPLICEPOINT;
	...
	rtDeliver = rtNext = ((REFERENCE_TIME)0x7fffffff<<32)|((REFERENCE_TIME)0xffffffff);
	for ( nIndex = 0; nIndex < This->avih.dwStreams; nIndex++ )
	{
		rtCur = This->pStreamsBuf[nIndex].rtCur;
		TRACE("stream %u - %lu,%u\n",nIndex,(unsigned long)(This->pStreamsBuf[nIndex].rtCur*1000/QUARTZ_TIMEUNITS),This->pStreamsBuf[nIndex].cIndexCur);
		if (This->pStreamsBuf[nIndex].strh.fccType == PARSER_auds)
			rtCur -= AUDIO_TIME;
		if ( rtDeliver > rtCur &&
	...
	if ( This->pIndexEntriesBuf != NULL )
	{
		...
		*pllStart = (LONGLONG)pStream->pIndexEntries[pStream->cIndexCur].dwChunkOffset + This->movieChunkOfs + 8;
		*plLength = (LONG)pStream->pIndexEntries[pStream->cIndexCur].dwChunkLength;
		*prtStart = rtNext;
		*prtStop = rtNext;

#ifdef KEYFRAMES
		if (pStream->pIndexEntries[pStream->cIndexCur].dwFlags & AVIIF_KEYFRAME)
#endif
			*pdwSampleFlags |= AM_SAMPLE_SPLICEPOINT;
		...
		*prtStop = rtNext;
#ifdef NO_AUDIO
		if (pStream->strh.fccType == PARSER_auds) goto do_it_again; /* skip audio */
#endif
#ifdef NO_VIDEO
		if (pStream->strh.fccType == PARSER_vids) goto do_it_again; /* skip video */
#endif
	}
	else
	{
...
static HRESULT CAVIParseImpl_GetCurPos( CParserImpl* pImpl, const GUID* pTimeFormat, DWORD nStreamIndex, LONGLONG* pllPos )
{
	CAVIParseImpl*	This = (CAVIParseImpl*)pImpl->m_pUserData;

	TRACE("(%p,%s,%u,...)\n",This,debugstr_guid(pTimeFormat),nStreamIndex);

	if ( This == NULL || This->pStreamsBuf == NULL )
		return E_UNEXPECTED;

#if 0
	if ( IsEqualGUID(pTimeFormat,&TIME_FORMAT_MEDIA_TIME) )
	{
		CAVIParseStream* pStream = &This->pStreamsBuf[nStreamIndex];
		/* FIXME: should we return the pos for the currently playing sample, not the next? */
		REFERENCE_TIME rtCur = pStream->rtCur;
		*pllPos = rtCur;
		TRACE("curpos %f (%lld)\n",(((double)*pllPos)/QUARTZ_TIMEUNITS), *pllPos);
		return S_OK;
	}
#endif
