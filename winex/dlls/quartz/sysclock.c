...
HRESULT QUARTZ_CreateReferenceClock(IUnknown* punkOuter,void** ppobj,pRefClock_GetTime refTime)
{
	CSystemClock*	psc;
	HRESULT	hr;
...
{
	return QUARTZ_CreateReferenceClock(punkOuter, ppobj, NULL);
}
...
DWORD WINAPI IReferenceClock_TimerEntry( LPVOID lpvParam )
{
	...
	while ( 1 )
	{
		...
		TRACE( "catch an event / timeout %u\n", dwTimeout );
...
IReferenceClock_fnGetTime(IReferenceClock* iface,REFERENCE_TIME* prtTime)
{
	...
	if ( This->m_pGetTime )
		return (*This->m_pGetTime)(This->unk.punkControl, prtTime);
...
HRESULT CSystemClock_InitIReferenceClock( CSystemClock* psc )
{
	...
	CRITICAL_SECTION_DEFINE( &psc->m_csClock );
	psc->m_dwTimeLast = psc->m_pGetTime ? 0 : GetTickCount();
