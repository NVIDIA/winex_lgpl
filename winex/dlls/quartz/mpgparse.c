...
static HRESULT CMPGParseImpl_InitParser( CParserImpl* pImpl, ULONG* pcStreams )
{
	...
	hr = IAsyncReader_SyncRead( pImpl->m_pReader, This->llStart, 10, hdrbuf );
	if ( FAILED(hr) )
		return hr;
	if ( hr != S_OK )
		return E_FAIL;

	if ( hdrbuf[0] == 'I' && hdrbuf[1] == 'D' &&
	     hdrbuf[2] == '3') {
		/* BYTE flags = hdrbuf[5]; */
		DWORD size =
		    ((hdrbuf[6] & 0x7f) << 21) |
		    ...
		    ((hdrbuf[8] & 0x7f) << 7) |
		    ((hdrbuf[9] & 0x7f));
		/* FIXME: if the tag is "unsynchronized" (bit 7 in flags is set),
		 * skipping the tag may be a little more complicated... */
		This->llStart += 10 + size;

		hr = IAsyncReader_SyncRead( pImpl->m_pReader, This->llStart, 8, hdrbuf );
		if ( FAILED(hr) )
			return hr;
		if ( hr != S_OK )
			return E_FAIL;
		 ...
		 hdrbuf[2] == 0x01 && hdrbuf[3] == 0xb3 )
	{
		...
		This->llPosNext = This->llStart;
		This->llLength = 0;
		IAsyncReader_Length( pImpl->m_pReader, &This->llLength, &dummy );
...
static HRESULT CMPGParseImpl_GetStreamType( CParserImpl* pImpl, ULONG nStreamIndex, AM_MEDIA_TYPE* pmt )
{
	...
	case MPGPayload_Video:
		hr = CMPGParseImpl_SyncReadPayload(
			pImpl, This, payloadtype, This->llStart, 140+10, hdrbuf );
		if ( FAILED(hr) )
			return hr;
		if ( hr != S_OK )
			return E_FAIL;
	...
	case MPGPayload_Audio:
		hr = CMPGParseImpl_SyncReadPayload(
			pImpl, This, payloadtype, This->llStart, 4, hdrbuf );
		if ( FAILED(hr) )
			return hr;
		if ( hr != S_OK )
			return E_FAIL;
	...
	*pdwSampleFlags = AM_SAMPLE_SPLICEPOINT;
	...
	if ( This->bRawPayload )
	{
		if ( This->dwPayloadBlockSizeMax == 0 ||
			...
			return E_UNEXPECTED;
		if ( This->llPosNext >= This->llLength )
			return S_FALSE; /* end of stream */
		*pnStreamIndex = 0;
		*pllStart = This->llPosNext;
		*plLength = This->dwPayloadBlockSizeMax;
		if ( This->llPosNext + *plLength > This->llLength )
			*plLength = This->llLength - This->llPosNext;
		*prtStart = 0;
		*prtStop = 0;
