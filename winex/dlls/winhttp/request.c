                                ...
                                LPVOID optional, DWORD optional_len, DWORD total_len, DWORD_PTR context )
{
    ...
    TRACE("%p, %s, 0x%x, %tu, %u, %u, %tx\n",
          hrequest, debugstr_w(headers), headers_len, optional, optional_len, total_len, context);
...
static BOOL read_reply( request_t *request )
{
    ...
    len = max( buflen + (crlf_len * 2) + 1, INITIAL_HEADER_BUFFER_LEN );
    ...
    memcpy( raw_headers + buflen + crlf_len - 1, crlf, sizeof(crlf) );
    ...
    for (;;)
    {
        ...
        while (len - offset < buflen + (crlf_len * 2) + 1)
        {
            WCHAR *tmp;
            len *= 2;
    ...
    /* always terminate the header with an empty line since many apps expect this to be presnt,
       but not all servers respond with it. */
...
static BOOL receive_response( request_t *request, BOOL async )
{
    ...
    for (;;)
    {
        ...
        {
            /* HTTP_STATUS_NO_CONTENT indicates no body data, set chunked_eof to prevent any more data reads */
            request->read_chunked_eof = TRUE;
        }
        break;
    }
...
static CONST_VTBL struct IWinHttpRequestVtbl winhttp_request_vtbl =
{
