...
BOOL free_handle( HINTERNET hinternet )
{
    ...
    if ((handle > 0) && (handle <= max_handles))
    {
        ...
        {
            hdr = handles[handle];
            TRACE("destroying handle 0x%tx for object %p\n", handle + 1, hdr);
            handles[handle] = NULL;
            ret = TRUE;
        }
    }
    ...
    if (hdr)
    {
        /* release one reference for each of this handle's children.  Note that each child also
           owns exactly one reference to the parent and that the app still owns a reference to
           each child.  Each child's reference to the parent will only be released when the
           child object is fully destroyed.  Also note that freeing the child object here would
           be an error because the app still owns a handle (and therefore a reference) to each
           child object.  Since freeing the child also removes the handle from the table, doing
           so will cause future lookups to fail which will in turn cause a failure when the app
           tries to close the handle later on. */
        LIST_FOR_EACH_ENTRY_SAFE( child, next, &hdr->children, object_header_t, entry )
        {
            TRACE("releasing child handle %p for parent handle 0x%tx\n", child->handle, handle + 1);
