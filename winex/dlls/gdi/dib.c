    ...
    UINT coloruse)   /* [in]  RGB or palette index */
{
    ...
    if (bits && lines)
    {
		/* DDP - early quit if source data not valid */
		if (bmp && bmp->dib && bmp->dib->dsBm.bmBits == NULL)
		{	
			ERR("GetDIBits source bmBits is NULL\n");
			GDI_ReleaseObj( hdc );
			GDI_ReleaseObj( hbitmap );
			return 0;		
		}
                            ...
                            UINT coloruse )
{
    ...
    if (fColor) {
        ...
        handle = CreateBitmap( width, height, GetDeviceCaps( dc, PLANES ),
