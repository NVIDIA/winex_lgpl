...
UINT MIBTypeFromIFType(UINT ifType)
{
    switch (ifType)
    {
        ...
        case IF_TYPE_PROP_POINT2POINT_SERIAL:
            return MIB_IF_TYPE_PPP;

        case IF_TYPE_SOFTWARE_LOOPBACK:
            return MIB_IF_TYPE_LOOPBACK;

        case IF_TYPE_FDDI:
            return MIB_IF_TYPE_FDDI;

        case IF_TYPE_SLIP:
            return MIB_IF_TYPE_SLIP;
...
static int compareInterfaceIndices(const void *_d1, const void *_d2)
{
    const IfSortData *  d1 = (const IfSortData *)_d1;
    const IfSortData *  d2 = (const IfSortData *)_d2;


    /* the types match => maintain the original enumeration order. */
    if (d1->type == d2->type)
        return d1->originalOrder - d2->originalOrder;

    /* make sure to sort all of the 'other' type interfaces at the end of the list in case we
       couldn't categorize some of them. */
    if (d1->type == IF_TYPE_OTHER)
        return 1;

    else if (d2->type == IF_TYPE_OTHER)
        return -1;

    return d1->type - d2->type;
}

void sortInterfaceIndicesByType(InterfaceIndexTable *table)
{
    INT         i;
    IF_INDEX    index;
    IfSortData *data;
    ...
    /****** allocate a sorting buffer and collect info ******/
    data = HeapAlloc(GetProcessHeap(), 0, sizeof(IfSortData) * table->numIndexes);

    if (data == NULL)
    {
        ERR("failed to allocate a sorting buffer for interface indices\n");
        return;
    }

    /* collect each interface's type, name, and original enumeration order.  The name will only
       be used for debugging purposes later.  The original enumeration order value is used to
       maintain the relative order of interfaces with the same type. */
    for (i = 0; i < table->numIndexes; i++)
    {
        index = table->indexes[i];

        data[i].index = index;
        data[i].originalOrder = i;
        getInterfacePhysicalByIndex(index, NULL, NULL, &data[i].type);
        getInterfaceNameByIndex(index, data[i].name);
    }


    /****** sort the interfaces ******/
    qsort(data, table->numIndexes, sizeof(IfSortData), compareInterfaceIndices);


    /****** write the sorted indices back to the table ******/
    TRACE("dumping sorted interface list:\n");

    for (i = 0; i < table->numIndexes; i++)
    {
        table->indexes[i] = data[i].index;
        TRACE("    interface %u has name '%s'\n", table->indexes[i], data[i].name);
    }
...
DWORD get_interface_indices( BOOL skip_loopback, InterfaceIndexTable **table )
{
    ...
    for (p = indices; p->if_name; p++)
    {
        ...
        if (!USE_ETH_ADAPTER(p->if_name)) continue;
        count++;
    }
    ...
    if (table)
    {
        ...
        {
            if (skip_loopback && isIfIndexLoopback( p->if_index )) continue;
            if (!USE_ETH_ADAPTER(p->if_name))
            {
                TRACE("skipping interface '%s'\n", p->if_name);
                continue;
            }
...
static BOOL isInternetInterface(int fd, const char *name)
{
  BOOL ret = TRUE;
  ...
  if (name) {
    ...
    ret=ioctl(fd, SIOCGIFADDR, &ifr) == 0;
  }
  return ret;
}
 ...
 PDWORD type)
{
  ...
  fd = socket(PF_INET, SOCK_DGRAM, 0);
  if (fd != -1) {
    if (isLoopbackInterface(fd, name) || !isInternetInterface(fd, name)) {
      *type = IF_TYPE_SOFTWARE_LOOPBACK;
      if (addr != NULL && len != NULL)
      {
        memset(addr, 0, *len);
...
static UINT getInterfaceType(UINT type, const char *name)
{
#if defined(HAVE_NET_IF_TYPES_H)
# define CASE(native, ifType) case native: TRACE("found a '%s' interface type for '%s'\n", &(#native)[4], name); return ifType

    switch (type)
    {
        CASE(IFT_1822,              IF_TYPE_REGULAR_1822);
        CASE(IFT_HDH1822,           IF_TYPE_HDH_1822);
        CASE(IFT_X25DDN,            IF_TYPE_DDN_X25);
        CASE(IFT_X25,               IF_TYPE_RFC877_X25);
        CASE(IFT_ETHER,             IF_TYPE_ETHERNET_CSMACD);
        CASE(IFT_ISO88023,          IF_TYPE_IS088023_CSMACD);
        CASE(IFT_ISO88024,          IF_TYPE_ISO88024_TOKENBUS);
        CASE(IFT_ISO88025,          IF_TYPE_ISO88025_TOKENRING);
        CASE(IFT_ISO88026,          IF_TYPE_ISO88026_MAN);
        CASE(IFT_STARLAN,           IF_TYPE_STARLAN);
        CASE(IFT_P10,               IF_TYPE_PROTEON_10MBIT);
        CASE(IFT_P80,               IF_TYPE_PROTEON_80MBIT);
        CASE(IFT_HY,                IF_TYPE_HYPERCHANNEL);
        CASE(IFT_FDDI,              IF_TYPE_FDDI);
        CASE(IFT_LAPB,              IF_TYPE_LAP_B);
        CASE(IFT_SDLC,              IF_TYPE_SDLC);
        CASE(IFT_T1,                IF_TYPE_DS1);
        CASE(IFT_CEPT,              IF_TYPE_E1);
        CASE(IFT_ISDNBASIC,         IF_TYPE_BASIC_ISDN);
        CASE(IFT_ISDNPRIMARY,       IF_TYPE_PRIMARY_ISDN);
        CASE(IFT_PTPSERIAL,         IF_TYPE_PROP_POINT2POINT_SERIAL);
        CASE(IFT_PPP,               IF_TYPE_PPP);
        CASE(IFT_LOOP,              IF_TYPE_SOFTWARE_LOOPBACK);
        CASE(IFT_EON,               IF_TYPE_EON);
        CASE(IFT_XETHER,            IF_TYPE_ETHERNET_3MBIT);
        CASE(IFT_NSIP,              IF_TYPE_NSIP);
        CASE(IFT_SLIP,              IF_TYPE_SLIP);
        CASE(IFT_ULTRA,             IF_TYPE_ULTRA);
        CASE(IFT_DS3,               IF_TYPE_DS3);
        CASE(IFT_SIP,               IF_TYPE_SIP);
        CASE(IFT_FRELAY,            IF_TYPE_FRAMERELAY);
        CASE(IFT_RS232,             IF_TYPE_RS232);
        CASE(IFT_PARA,              IF_TYPE_PARA);
        CASE(IFT_ARCNET,            IF_TYPE_ARCNET);
        CASE(IFT_ARCNETPLUS,        IF_TYPE_ARCNET_PLUS);
        CASE(IFT_ATM,               IF_TYPE_ATM);
        CASE(IFT_MIOX25,            IF_TYPE_MIO_X25);
        CASE(IFT_SONET,             IF_TYPE_SONET);
        CASE(IFT_X25PLE,            IF_TYPE_X25_PLE);
        CASE(IFT_ISO88022LLC,       IF_TYPE_ISO88022_LLC);
        CASE(IFT_LOCALTALK,         IF_TYPE_LOCALTALK);
        CASE(IFT_SMDSDXI,           IF_TYPE_SMDS_DXI);
        CASE(IFT_FRELAYDCE,         IF_TYPE_FRAMERELAY_SERVICE);
        CASE(IFT_V35,               IF_TYPE_V35);
        CASE(IFT_HSSI,              IF_TYPE_HSSI);
        CASE(IFT_HIPPI,             IF_TYPE_HIPPI);
        CASE(IFT_MODEM,             IF_TYPE_MODEM);
        CASE(IFT_AAL5,              IF_TYPE_AAL5);
        CASE(IFT_SONETPATH,         IF_TYPE_SONET_PATH);
        CASE(IFT_SONETVT,           IF_TYPE_SONET_VT);
        CASE(IFT_SMDSICIP,          IF_TYPE_SMDS_ICIP);
        CASE(IFT_PROPVIRTUAL,       IF_TYPE_PROP_VIRTUAL);
        CASE(IFT_PROPMUX,           IF_TYPE_PROP_MULTIPLEXOR);
        CASE(IFT_GIF,               IF_TYPE_TUNNEL);
        CASE(IFT_FAITH,             IF_TYPE_TUNNEL);
        CASE(IFT_STF,               IF_TYPE_TUNNEL);
        CASE(IFT_L2VLAN,            IF_TYPE_L2_VLAN);
        CASE(IFT_IEEE8023ADLAG,     IF_TYPE_L3_IPVLAN);
        CASE(IFT_IEEE1394,          IF_TYPE_IEEE1394);
        CASE(IFT_BRIDGE,            IF_TYPE_TUNNEL);
        CASE(IFT_ENC,               IF_TYPE_TUNNEL);
        CASE(IFT_PFLOG,             IF_TYPE_OTHER);
        CASE(IFT_PFSYNC,            IF_TYPE_OTHER);
        CASE(IFT_CARP,              IF_TYPE_OTHER);
        CASE(IFT_CELLULAR,          IF_TYPE_OTHER);
        default:
            TRACE("got an unknown interface type 0x%02x for interface '%s'\n", type, name);
            return IF_TYPE_OTHER;
    }
#undef CASE
#else
 ...
 PDWORD type)
{
  ...
  for (p = buf; !found && p < buf + mibLen; p += ifm->ifm_msglen) {
    ...
    *type = getInterfaceType(sdl->sdl_type, name);
    ret = NO_ERROR;
...
DWORD getInterfaceEntryByName(const char *name, PMIB_IFROW entry)
{
  ...
  if (getInterfacePhysicalByName(name, &len, addr, &type) == NO_ERROR) {
    ...
    *assigner = 0;
    ...
    entry->dwType = MIBTypeFromIFType(type);
