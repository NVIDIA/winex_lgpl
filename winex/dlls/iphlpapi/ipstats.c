...
#ifdef LIMITADAPTERS_ENABLE
          getInterfaceNameByIndex(rtm->rtm_index, name);
          if (!USE_ETH_ADAPTER(name))
          {
...
static struct pid_map *get_pid_map( unsigned int *num_entries )
{
    ...
    {
        req->flags = TH32CS_SNAPPROCESS;
        req->inherit = 0;
        req->pid = 0;
        if (!(ret = wine_server_call( req )))
            snapshot = extend_server_handle( reply->handle );
    }
    SERVER_END_REQ;
    ...
    *num_entries = 0;
    while (ret == STATUS_SUCCESS)
    {
        ...
        {
            req->handle = wine_server_handle( snapshot );
            req->reset = (i == 0);
            if (!(ret = wine_server_call( req )))
            {
                if (i >= count)
