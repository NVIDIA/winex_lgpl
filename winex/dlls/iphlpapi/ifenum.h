 ...
 * and type as one of the IF_TYPE_* types.
...
/* retrieves the MIB_IF_TYPE_* equivalent for an IF_TYPE_* interface type name.  Note that this
 * may map several different IF_TYPE_* names to a single MIB_IF_TYPE_* name.
 */
UINT MIBTypeFromIFType(UINT ifType);

/* sorts an InterfaceIndexTable object by the IF_TYPE_* that each interface represents.  The
 * sorting will be performed in-place and may require the type information for several interfaces
 * to be gathered multiple times.
 */
